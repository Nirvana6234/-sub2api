...
/** 成本采集状态。 */
export type CostStatus = 'admin_accounted' | 'complete' | 'partial' | 'invalid_rate' | 'missing' | 'cached_fallback'

/** 五项核心指标的实时数据。 */
export interface DashboardMetricsResponse {
  todayProfitUSD: Money
  todayProfitCNY: Money
  siteBalanceUSD: Money
  siteBalanceCNY: Money
  todayPurchaseCNY: Money
  netProfitCNY: Money
  upstreamBalanceCNY: Money
  costStatus: CostStatus
  /** 本次计算实际使用的 USD→CNY 汇率，与快照一起持久化。 */
  usdToCnyRate: number
  groupCount: number
}
...
export interface BalanceFilterConfig {
  ...
  /**
   * 营收/站点余额从 USD 折算到 CNY 的倍率，与余额筛选同源存储。
   * 可选：调用方只改筛选条件时不传，后端保留库里原值（不会被重置成默认值）。
   * 传 0 或负数等同于不传。
   */
  usdToCnyRate?: number
}
