import type {
  ...
  UpstreamKeyMultiplierHistoryPoint,
...
export const getUpstreamKeyMultiplierHistory = async (
  targetId: string,
  range: 'hour' | 'day' | 'week',
): Promise<UpstreamKeyMultiplierHistoryPoint[]> =>
  requestJson<UpstreamKeyMultiplierHistoryPoint[]>(
    `/connection-health/targets/${encodeURIComponent(targetId)}/upstream-key-multiplier-history?range=${range}`,
  )
...
export const setManualUpstreamKeyMultiplier = async (targetId: string, multiplier: number): Promise<void> => {
  await requestJson(`/connection-health/targets/${encodeURIComponent(targetId)}/manual-upstream-key-multiplier`, {
    method: 'PUT',
    body: JSON.stringify({ multiplier }),
  })
}

export const clearManualUpstreamKeyMultiplier = async (targetId: string): Promise<void> => {
  await requestJson(`/connection-health/targets/${encodeURIComponent(targetId)}/manual-upstream-key-multiplier`, {
    method: 'DELETE',
  })
}
...
export const setAdminGroupStatus = async (adminGroupId: string, active: boolean): Promise<void> => {
  await requestJson(`/connection-health/admin-groups/${encodeURIComponent(adminGroupId)}/status`, {
    method: 'PUT',
