...
import { clearManualUpstreamKeyMultiplier, setAdminGroupStatus, setManualUpstreamKeyMultiplier } from '../api/connectionHealth'
...
import ManualUpstreamKeyMultiplierDialog from '../components/dashboard/ManualUpstreamKeyMultiplierDialog.vue'
import type { ManualUpstreamKeyMultiplierTarget } from '../components/dashboard/ManualUpstreamKeyMultiplierDialog.vue'
import UpstreamKeyMultiplierHistoryDialog from '../components/dashboard/UpstreamKeyMultiplierHistoryDialog.vue'
...
const statusChangingGroupId = ref('')
...
const toggleAdminGroupStatus = async (group: AdminGroupHealth, active: boolean) => {
  if (!group.statusMutable || statusChangingGroupId.value) return
  statusChangingGroupId.value = group.id
  try {
    await setAdminGroupStatus(group.id, active)
    await loadAll({ silent: true })
  } catch (error) {
    errorKey.value = error instanceof Error ? error.message : 'admin.connectionHealth.errors.request'
  } finally {
    statusChangingGroupId.value = ''
  }
}
...
const manualMultiplierDialogOpen = ref(false)
const manualMultiplierTarget = ref<ManualUpstreamKeyMultiplierTarget | null>(null)
const manualMultiplierSaving = ref(false)
const manualMultiplierError = ref('')
const multiplierHistoryOpen = ref(false)
const multiplierHistoryTarget = ref<AdminGroupAccount | null>(null)

const openMultiplierHistory = (account: AdminGroupAccount) => {
  multiplierHistoryTarget.value = account
  multiplierHistoryOpen.value = true
}

const openManualMultiplier = (account: AdminGroupAccount) => {
  if (account.upstreamKeyGroupMultiplierSource === 'detected') return
  manualMultiplierError.value = ''
  manualMultiplierTarget.value = {
    targetId: account.targetId,
    accountName: account.name || account.id,
    multiplier: account.upstreamKeyGroupMultiplier,
    isManual: account.upstreamKeyGroupMultiplierSource === 'manual',
  }
  manualMultiplierDialogOpen.value = true
}

const saveManualMultiplier = async (multiplier: number) => {
  if (!manualMultiplierTarget.value || manualMultiplierSaving.value) return
  manualMultiplierSaving.value = true
  manualMultiplierError.value = ''
  try {
    await setManualUpstreamKeyMultiplier(manualMultiplierTarget.value.targetId, multiplier)
    manualMultiplierDialogOpen.value = false
    await loadAll({ silent: true })
  } catch (error) {
    manualMultiplierError.value = readableMessage(error instanceof Error ? error.message : 'admin.connectionHealth.errors.request')
  } finally {
    manualMultiplierSaving.value = false
  }
}

const clearManualMultiplier = async () => {
  if (!manualMultiplierTarget.value || !manualMultiplierTarget.value.isManual || manualMultiplierSaving.value) return
  manualMultiplierSaving.value = true
  manualMultiplierError.value = ''
  try {
    await clearManualUpstreamKeyMultiplier(manualMultiplierTarget.value.targetId)
    manualMultiplierDialogOpen.value = false
    await loadAll({ silent: true })
  } catch (error) {
    manualMultiplierError.value = readableMessage(error instanceof Error ? error.message : 'admin.connectionHealth.errors.request')
...
const togglePolicyEnabled = async (policy: ConnectionHealthPolicy) => {
  await savePolicy({
    ...
    priorityStrategy: policy.priorityStrategy ?? 'price',
...
<template>
  <div class="space-y-5">
    ...
    <section class="overflow-hidden rounded-lg border border-border/60 bg-card text-card-foreground shadow-sm">
      ...
      <div v-else class="grid min-h-[34rem] lg:grid-cols-[19rem_minmax(0,1fr)]">
            ...
            <template v-else>
              ...
              >
                <span
                  ...
                  :class="groupMonitoringEnabled(group) ? 'bg-emerald-500' : group.priorityMode === 'multiplier' || group.priorityMode === 'auto' ? 'bg-primary' : 'bg-muted-foreground/35'"
                />
                <span class="min-w-0 flex-1">
                  <span class="flex items-center justify-between gap-2">
                    <span class="truncate text-sm font-medium text-foreground">{{ group.name }}</span>
                    <ArrowDownUp v-if="group.priorityMode === 'multiplier' || group.priorityMode === 'auto'" class="h-3.5 w-3.5 shrink-0 text-primary" />
                  </span>
        ...
        <AdminGroupHealthDetail
          v-else
          :group="selectedGroup"
          :status-changing="statusChangingGroupId === selectedGroup.id"
          @setup="openSetup"
          @probe="onProbeAccount"
          @view-events="onViewEventsAccount"
          @edit-upstream-key-multiplier="openManualMultiplier"
          @view-upstream-key-multiplier-history="openMultiplierHistory"
          @toggle-status="(active) => toggleAdminGroupStatus(selectedGroup!, active)"
        />
      </div>
    </section>
    ...
    <ManualUpstreamKeyMultiplierDialog
      :open="manualMultiplierDialogOpen"
      :target="manualMultiplierTarget"
      :saving="manualMultiplierSaving"
      :error="manualMultiplierError"
      @close="manualMultiplierDialogOpen = false"
      @save="saveManualMultiplier"
      @clear="clearManualMultiplier"
    />
    <UpstreamKeyMultiplierHistoryDialog :open="multiplierHistoryOpen" :target-id="multiplierHistoryTarget?.targetId ?? ''" :account-name="multiplierHistoryTarget?.name || multiplierHistoryTarget?.id || ''" @close="multiplierHistoryOpen = false" />
