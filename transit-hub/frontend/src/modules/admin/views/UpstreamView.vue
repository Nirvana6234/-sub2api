...
import type { UpstreamGroupInfo, UpstreamMetricValue, UpstreamSite, UpstreamSiteForm, UpstreamSiteResponse, UpstreamStatus } from '../types/upstream'
...
const handleEditSite = (site: UpstreamSite) => {
  ...
  newSiteForm.value = {
    ...
    authMode: site.platform === 'sub2api' || site.requestedPlatform === 'sub2api' ? 'token' : 'password',
...
const onSiteSettingsSaved = (siteId: string, settings: { balanceThreshold: number | null; manualAccountingEnabled: boolean; manualGroupMultipliers?: Record<string, number> }) => {
...
const onSiteRecharged = (siteId: string, response: UpstreamSiteResponse) => {
  const site = upstreamSites.value.find(s => s.id === siteId)
  if (site) Object.assign(site, response)
}
