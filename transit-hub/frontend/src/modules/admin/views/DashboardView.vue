...
import { computeDelta, formatCny, formatUsd, formatDateTime } from '../utils/dashboard'
...
// key 必须与 cards 数组里实际发出的 key 一致（带币种后缀）。
// 这里参数是 string 而非 DashboardMetricKey，改名后 TS 不会报错，
// 只会表现为「点了没反应」，所以改卡片 key 时务必同步这里。
const handleMetricCardClick = (key: string) => {
  switch (key) {
    case 'todayProfitCNY':
      openGroupUsageToday()
      break
    case 'todayPurchaseCNY':
      openUpstreamKeyUsageToday()
      break
    case 'upstreamBalanceCNY':
...
const METRIC_META: Record<DashboardMetricKey, { icon: Component; labelKey: string; color: DashboardColorToken; formatter: (v: number) => string }> = {
  todayProfitUSD: { icon: TrendingUp, labelKey: 'admin.dashboard.metrics.todayProfit', color: 'primary', formatter: formatUsd },
  todayProfitCNY: { icon: TrendingUp, labelKey: 'admin.dashboard.metrics.todayProfit', color: 'primary', formatter: formatCny },
  siteBalanceUSD: { icon: Wallet, labelKey: 'admin.dashboard.metrics.siteBalance', color: 'accent', formatter: formatUsd },
  siteBalanceCNY: { icon: Wallet, labelKey: 'admin.dashboard.metrics.siteBalance', color: 'accent', formatter: formatCny },
  todayPurchaseCNY: { icon: ShoppingCart, labelKey: 'admin.dashboard.metrics.todayPurchase', color: 'warning', formatter: formatCny },
  netProfitCNY: { icon: PiggyBank, labelKey: 'admin.dashboard.metrics.netProfit', color: 'signal', formatter: formatCny },
  upstreamBalanceCNY: { icon: Landmark, labelKey: 'admin.dashboard.metrics.upstreamBalance', color: 'primary', formatter: formatCny },
}
...
const cards = computed<DashboardCoreCard[]>(() => {
  // 三张卡统一用 CNY：营收先按汇率折成人民币，才能和成本同币种相减。
  // 早前这里混用 todayProfitUSD 与 todayPurchaseCNY，两张卡币种不同无法对比，
  // 且净利润卡整个缺失——后端算好的 netProfitCNY 没有任何前端消费者。
  const result: DashboardCoreCard[] = (['todayProfitCNY', 'todayPurchaseCNY', 'netProfitCNY'] as DashboardMetricKey[]).flatMap((key) => {
    ...
    return [{
      ...
      icon: meta.icon,
      color: meta.color,
      value: meta.formatter(current.current),
      deltaDirection: delta.direction,
      deltaText: meta.formatter(Math.abs(delta.amount)),
      // key 已改成 CNY 后缀，这里必须同步，否则营收卡不再可点（TS 抓不到字符串错配）。
      clickable: key === 'todayProfitCNY' || key === 'todayPurchaseCNY',
      negativeWhenUp: key === 'todayPurchaseCNY',
    }]
  })
  return result
})
...
// 三项必须同币种（CNY），否则 profit = revenue - cost 又是 USD - CNY 混算。
// 营收取 todayProfitCNY（后端已按当日汇率折算），不要用 todayProfitUSD。
const periodTotals = computed(() => ({
  revenue: sumSeries('todayProfitCNY'),
  cost: sumSeries('todayPurchaseCNY'),
  profit: sumSeries('netProfitCNY'),
}))
...
const performanceChartOption = computed<EChartsCoreOption>(() => {
  // 同一根 Y 轴上的柱子必须同币种，否则 USD 柱和 CNY 柱高低无可比性，
  // 视觉上仍是「巨亏」。compactCurrency 也是按 ¥ 格式化的。
  const revenue = selectedSeries('todayProfitCNY')
  const cost = selectedSeries('todayPurchaseCNY')
  const theme = chartTheme.value
  const commonSeries = (name: string, data: number[], color: string, formatter: (v: number) => string) => ({
    name,
    data,
    itemStyle: { color },
    tooltip: { valueFormatter: (value: number) => formatter(value) },
  })
  return {
    ...
    // 单根 Y 轴：营收与成本都取自 *CNY 字段，同币种就必须共用刻度。此前是左右
    // 双轴、左轴还按 $ 标注，两根轴刻度相差数倍，于是同样一天的营收柱和成本柱
    // 高低完全不可比——某天营收 ¥580、成本 ¥50 看着是「顶天的利润」，另一天
    // 营收 ¥230、成本 ¥2500 看着是「巨亏」，实际差距远没有视觉上那么悬殊。
    yAxis: {
      type: 'value',
      position: 'left',
      axisLabel: { color: theme.muted, formatter: (value: number) => `¥${numberFormatter.value.format(value)}` },
      splitLine: { lineStyle: { color: theme.border, type: 'dashed' } },
    },
    dataZoom: period.value === 'month' ? [{ type: 'inside', start: 0, end: 100 }] : [],
    series: [
      // 两条都用 formatCny：数据源是 todayProfitCNY / todayPurchaseCNY，
      // 用 formatUsd 会给人民币数值打上 $ 符号，比缺数字更容易误判。
      { ...commonSeries(t('admin.dashboard.metrics.todayProfit'), revenue.map(point => point.value), theme.primary, formatCny), type: 'bar', barMaxWidth: 18, itemStyle: { color: theme.primary, borderRadius: [3, 3, 0, 0] } },
      { ...commonSeries(t('admin.dashboard.metrics.todayPurchase'), cost.map(point => point.value), theme.warning, formatCny), type: 'bar', barMaxWidth: 18, itemStyle: { color: theme.warning, borderRadius: [3, 3, 0, 0] } },
    ],
  }
})
...
// 覆盖率是「上游余额能兑几成站点欠款」，两边必须同币种。
// 早前用 siteBalanceUSD 做分母 = CNY ÷ USD，算出的百分比在汇率 7 时被放大约 7 倍。
const siteBalance = computed(() => metric('siteBalanceCNY')?.current ?? 0)
const upstreamBalance = computed(() => metric('upstreamBalanceCNY')?.current ?? 0)
...
const averageDailyCost = computed(() => {
  const values = selectedSeries('todayPurchaseCNY').map(point => point.value)
...
const attentionItems = computed(() => {
  ...
  // 已不被任何策略探活的遗留目标：用中性色提示而不是告警色。它们不是故障，
  // 只是策略移除模型后留下的历史状态，需要运营决定是恢复探活还是清理掉。
  if ((healthSummary.value?.unmonitoredTargets ?? 0) > 0) {
    items.push({
      key: 'unmonitored',
      icon: Activity,
      title: t('admin.dashboard.attention.unmonitoredTitle'),
      description: t('admin.dashboard.attention.unmonitoredDescription'),
      count: healthSummary.value?.unmonitoredTargets ?? 0,
      ...
      <template v-else-if="metrics.length > 0">
        ...
        <section class="grid gap-4 xl:grid-cols-12">
          <article class="min-w-0 rounded-lg border border-border/60 bg-card p-4 shadow-sm sm:p-5 xl:col-span-8">
            ...
            <!-- 三格必须同币种（CNY）。早前 revenue 用 formatUsd 渲染，而 periodTotals.revenue
                 已是 CNY，等于给人民币数值打上 $ 符号——比缺数字更容易误判。 -->
          ...
          <aside class="min-w-0 rounded-lg border border-border/60 bg-card p-4 shadow-sm sm:p-5 xl:col-span-4">
            ...
            <div class="mt-6 divide-y divide-border/60 border-y border-border/60">
              <button type="button" class="flex w-full items-center justify-between gap-4 py-4 text-left" @click="openBalanceFilter">
                ...
                <dd class="font-semibold tabular-nums text-foreground">{{ formatUsd(siteBalance) }}</dd>
              </button>
