...
export interface TargetPolicyAssignmentSummary {
  ...
  priorityStrategy?: ConnectionHealthPriorityStrategy
...
export interface AdminGroupAccount {
  ...
  upstreamKeyGroupMultiplierSource?: 'detected' | 'cached' | 'manual' | string
...
export interface UpstreamKeyMultiplierHistoryPoint {
  observedAt: string
  multiplier: number
  samples: number
...
export interface ConnectionHealthStoredSummary {
  ...
  // 只剩历史状态、已不被任何启用策略探活的目标数。它们不计入上面的健康/告警/
  // 熔断数字，否则策略移除某个模型后，面板会长期挂着一片无人会去处理的假红。
  unmonitoredTargets: number
...
export interface PolicyInput {
  ...
  ownGroupName: string
  modelPattern?: string
  /** 探活方式："real_model" = 真实模型请求（消耗 token）；"models_endpoint" = GET /v1/models（零消耗）。空值由后端退回 real_model。 */
  probeMode?: string
  probeIntervalSeconds?: number
  failureThreshold?: number
  successThreshold?: number
...
export type ConnectionHealthPriorityMode = 'none' | 'multiplier' | 'auto'
export type ConnectionHealthPriorityStrategy = 'price' | 'balanced' | 'speed'
