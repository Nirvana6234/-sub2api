...
/** 仪表盘的核心指标（已按币种分离）。 */
// 每个键都必须同时存在于 DashboardMetricsResponse 和 DashboardTrendPoint，
// buildMetricData 用同一个 key 索引两者，缺一边 TS 会直接报错。
export type DashboardMetricKey =
  | 'todayProfitUSD' // 今日营收（USD 原值）
  | 'todayProfitCNY' // 今日营收（CNY，已乘汇率）
  | 'siteBalanceUSD' // 站点用户总余额（USD 原值）
  | 'siteBalanceCNY' // 站点用户总余额（CNY，已乘汇率）
  | 'todayPurchaseCNY' // 今日成本（CNY）
  | 'netProfitCNY' // 今日净利润（CNY）= 营收CNY - 成本CNY，同币种相减
  | 'upstreamBalanceCNY' // 上游总余额（CNY）
