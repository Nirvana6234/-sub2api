...
export function remoteActionLabelKey(remoteAction: string): { key: string; params?: Record<string, number | string> } | null {
  ...
  switch (remoteAction) {
    ...
    case 'sub2api_account_schedulable_false':
      return { key: `${prefix}.sub2apiSchedulableFalse` }
    case 'sub2api_account_schedulable_true':
      return { key: `${prefix}.sub2apiSchedulableTrue` }
    case 'sub2api_account_schedulable_false_failed':
      return { key: `${prefix}.sub2apiSchedulableFalseFailed` }
    case 'sub2api_account_schedulable_true_failed':
      return { key: `${prefix}.sub2apiSchedulableTrueFailed` }
