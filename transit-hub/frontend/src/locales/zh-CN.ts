export default {
  ...
  cta: {
    title: '准备好接管一切了吗？',
    subtitle: '加入成千上万使用共飞后台管理驱动其 API 基础设施的开发者行列。今天就免费开始吧。',
    deployBtn: '立即部署',
    salesBtn: '联系销售'
  },
  footer: {
    rights: '共飞运维团队。保留所有权利。'
  },
  auth: {
    backToHome: '返回主页',
    login: {
      title: '欢迎回来',
      subtitle: '输入您的邮箱和密码登录共飞后台管理',
      email: '邮箱',
  ...
  admin: {
    ...
    adminAccounts: {
      ...
      delete: {
        ...
        title: '删除 {name}',
        localDataWarning: '此工作区的所有共飞后台管理本地工作区数据将被永久删除，且无法恢复。',
    ...
    dashboard: {
      ...
      attention: {
        ...
        unmonitoredTitle: '目标已不在探活范围',
        unmonitoredDescription: '这些目标只剩历史状态，当前没有任何启用策略会探活它们；请确认是补回模型配置还是清理掉',
      ...
      balanceFilter: {
        ...
        add: '添加',
        usdToCnyRate: 'USD → CNY 汇率',
        usdToCnyRateHelp: '管理员站点的营收与用户余额按此倍率折算成人民币，才能与上游成本同币种比较。留空或填 0 表示保持当前值不变。',
        cancel: '取消',
        save: '保存',
        saving: '保存中...',
        loadError: '加载筛选配置失败。',
    ...
    purityCheck: {
      title: 'GPT-5.6 纯度检测',
      subtitle: '对已对接的 OpenAI 上游账号发起固定探针检测，判断上游实际提供的是不是申报的型号。检测会真实消耗上游 token，且同一时刻只能跑一个，多选后按队列串行执行。',
      refresh: '刷新',
      disclaimer:
        '检测不通过不等于中转主动掺水：官方风控、出口 IP、反代、并发或临时路由都可能让请求落到不同能力层。检测通过也不能排除透明代理或差异化路由。发生争议时建议降低并发、换时间/IP 复测，并要求服务方提供对应时段的可核验证据。',
      detectorUnavailable: '检测器旁路服务未部署或连不上，本页功能暂不可用。请检查 PURITY_CHECK_DETECTOR_URL 配置与 gpt56-detector 容器状态。',
      ...
      tiers: {
        low: '低档 · 快速初筛',
        medium: '中档 · 常规检测',
        high: '高档 · 含长上下文与原生 Codex 对照',
      },
      // logical 是档位表上的条数，http 是算上重试后的最坏请求数，两者差 3 倍，必须都报。
      tierCost: '{logical} 条探针，最多 {http} 次请求，约 {tokens} 输入 token',
      tiersUnavailable: '档位信息取不到（检测器不可达）。',

      claimedModelLabel: '申报型号',
      ...
      confirmLogical: '探针条数合计',
      confirmMaxHttp: '最多请求次数（含重试）',
      confirmTokens: '输入 token 估算',
      confirmSerialNote: '检测器同一时刻只能跑一个会话，任务会按提交顺序串行执行，排在后面的需要等待。',
      confirmCancel: '取消',
      confirmOk: '确认并开始',
      ...
      viewReport: '查看报告',
      delete: '删除',
      deleteConfirm: '确定删除「{name}」这条检测记录及其报告？此操作不可撤销。',
      retentionHint: '仅保留最近 {max} 份，超出的最旧记录自动删除',
      probeSuccess: '有效探针 {ok}/{total}',
      nonOfficialBadge: '（非官方档位）',

      reportTitle: '检测报告',
      reportEmpty: '没有报告内容。',
      reportJuice: 'Juice 预算指纹',
      reportFingerprint: '行为指纹指向',
      reportFingerprintUnclear: '证据不明确',
      reportMatches: '各型号匹配度 / 强指向阈值',
      reportMatchesNote: '百分比表示这批固定答案的分布与三种可信分布的相对接近程度，不是"请求有多少比例被路由到该模型"，也不是真实身份概率。',
      reportRaw: '展开完整报告原文',
      nonOfficialNote: '这次运行的参数不是官方档位，结论只显示参考匹配度，不构成正式的"强烈指向"判定。',
      ...
      errors: {
        ...
        unknown: '发生未知错误。',
        notFound: '任务不存在。',
        noCurrentAccount: '请先选择一个 admin workspace。',
        detectorUnavailable: '检测器不可用。',
        detectorBusy: '检测器正忙，请稍后重试。',
        targetNotFound: '账号不存在或已被移除。',
        targetIneligible: '所选账号不支持该检测。',
        invalidTier: '档位无效。',
        invalidModel: '申报型号只能是 Sol、Terra 或 Luna。',
        accountsFetch: '读取账号列表失败。',
        credential: '解析上游凭据失败，无法发起检测。',
    ...
    connectionHealth: {
      ...
      manualMultiplierDialog: {
        title: '手动填写上游 API Key 倍率',
        multiplier: '倍率',
        invalid: '请输入 0 到 1000000 之间的有效数字。',
        clear: '清除手动值',
        cancel: '取消',
        save: '保存',
        saving: '保存中...',
      ...
      groupDetail: {
        multiplierPriority: '上游低价优先',
        autoPriority: '自动评分 · {strategy}',
        priorityStrategies: {
          price: '低价优先',
          balanced: '兼容模式',
          speed: '速度优先'
        ...
        autoSignals: {
          usageP95: 'TTFT P95 {value} ms · {count} 条',
          usageFallback: 'TTFT 样本不足（{count}/3）',
          schedulerScore: '调度参考分 {value}',
          concurrency: '并发 {current}/{limit}'
        },
        upstreamMultiplierPending: '关联后展示倍率',
        upstreamMultiplierManual: '手动',
        upstreamMultiplierCached: '缓存',
        editUpstreamMultiplier: '填写上游 API Key 倍率',
        models: {
          empty: '该目标还没有模型探活结果。',
          latency: '延迟 {value} ms',
          lastProbe: '最近 {value}',
      ...
      setup: {
        ...
        strategy: {
          ...
          options: {
            multiplier: {
              title: '倍率优先',
              description: '读取账号实际绑定的上游 API Key 倍率；倍率越低，上游优先级越高。'
            },
            auto: {
              title: '自动评分',
              description: '按低价、兼容或速度策略评分；暂停、禁用和不可调度目标统一置底。'
            },
            multiplierOnly: {
              title: '仅倍率优先级',
              description: '只按真实上游 API Key 倍率调整优先级，不发起模型探活或远端动作。'
            },
            stable: {
              title: '稳定优先',
        ...
        confirm: {
          ...
          enabled: '已启用',
          disabled: '未启用',
          multiplierRule: '优先级规则：可用目标按所选评分模式排序；暂停、禁用、不可调度或权重为 0 的目标统一置底。Sub2API 在每个分组内独立排序，检测到人工修改后停止覆盖。',
          multiplierOnlyRule: '仅倍率规则：不发起模型探活，但会使用已有健康状态把不可用目标置底；其余目标按真实上游 API Key 倍率从低到高排序。停用或解绑后恢复接管前优先级，人工修改仍受冲突保护。'
        },
        back: '上一步',
        next: '下一步',
        save: '保存分组策略'
      ...
      remoteActions: {
        ...
        sub2apiSchedulableFalse: 'Sub2API 账号已标记为不可调度',
        sub2apiSchedulableTrue: 'Sub2API 账号已恢复可调度',
        sub2apiSchedulableFalseFailed: 'Sub2API 账号标记为不可调度失败',
        sub2apiSchedulableTrueFailed: 'Sub2API 账号恢复可调度失败',
      ...
      policyDrawer: {
        ...
        modelTargetsLabelLightweight: '健康追踪标签（可选）',
        autoScoreUnavailableLightweight: '轻量存活检测不提供 TTFT 数据，无法使用自动评分',
        probeModeLabel: '探活方式',
        probeModes: {
          real_model: {
            title: '真实模型请求',
            description: '调用模型接口，最多消耗 1 token，提供 TTFT 数据'
          },
          models_endpoint: {
            title: '轻量存活检测',
            description: 'GET /v1/models 验证 key 有效性，零 token 消耗'
          }
        },
        addModelTarget: '添加模型',
        ...
        save: '保存策略',
        tooltips: {
          probeMode: '真实模型请求会调用 chat/completions 接口消耗至少 1 个 token（提供 TTFT 数据用于自动评分）；轻量存活检测只请求 GET /v1/models 验证 key 和端点可达性，不消耗任何 token，但无法检测具体模型的可用性、也没有 TTFT 数据。',
      ...
      modal: {
        ...
        form: {
          ...
          refreshTokenPlaceholder: '粘贴 refresh_token；Access Token 失效或临期时用于刷新',
          tokenType: 'Token Type',
          tokenTypePlaceholder: '默认 Bearer',
          tokenHelp: '系统先验证现有 access_token，避免失效的 refresh_token 阻塞有效会话；JWT 过期时间会自动识别，失效或临期时再刷新。',
          userId: '用户 ID',
      ...
      siteSettings: {
        ...
        saving: '保存中...',
        cancel: '取消',
        manualAccounting: '手工余额账本',
        manualAccountingHelp: '适用于无法读取上游余额的站点。首次录入可作为历史余额；后续同步会按 Sub2API 分组实际用量和当前分组倍率计算消费。',
        rechargeAmountPlaceholder: '充值金额',
        rechargeNotePlaceholder: '备注（可选）',
        addRecharge: '记录充值余额',
        rechargeInvalid: '请输入大于 0 的充值金额。',
        rechargeLoadError: '充值记录加载失败。',
        rechargeNoNote: '未填写备注',
        rechargeEmpty: '尚未记录充值余额。',
        manualGroupMultipliers: '手工分组倍率',
        manualGroupMultipliersHelp: '当该站点无法读取账户资料或分组时，系统只会按这里填写的倍率核算可读取的 API Key 用量；未填写的分组不会按 1 倍猜测扣费。',
        addManualGroupMultiplier: '添加分组',
        removeManualGroupMultiplier: '删除分组',
        manualGroupNamePlaceholder: 'Sub2API 分组名称',
        manualMultiplierPlaceholder: '倍率，例如 0.08'
      },
      viewMode: {
        list: '列表模式',
        card: '卡片模式'
      ...
      errors: {
        ...
        auth: '登录失败，请检查账号或密码。',
        adminRequired: 'Token 有效，但账户不是 Sub2API admin。管理功能需要管理员 Token 或 Admin API Key。',
        interactiveLoginRequired: '该站点启用了 Turnstile，请在官网浏览器登录后粘贴会话 Token，或使用 Admin API Key。',
    ...
    latencyCompensation: {
      title: '延迟补贴',
      subtitle: '给指定时间段内首字节响应过慢的请求退款——按成本价收费，差价退给用户。每个任务是一套独立的阈值/比例配置，可以手动运行，也可以开自动执行。',
      tasksTitle: '补贴任务',
      newTask: '新建任务',
      editTask: '编辑任务',
      taskName: '任务名称',
      taskNamePlaceholder: '例如：工作日夜间补贴',
      taskSummary: '每天 {windowStart}~{windowEnd} · 阈值 {seconds} 秒 · 退款比例 {ratio}%',
      autoBadge: '自动 · 每天 {time}',
      manualBadge: '手动',
      run: '运行',
      runTitle: '运行任务：{name}',
      runHint: '先预览，你确认之后才会真正发放，不会因为保存过任务或打开这个面板就自动执行。',
      tasksLoading: '任务加载中…',
      tasksEmpty: '还没有任务，点右上角"新建任务"建一个。',
      tasksLoadFailed: '加载任务失败',
      invalidName: '请填写任务名称',
      save: '保存',
      windowStart: '补贴时间段 · 起点',
      windowEnd: '补贴时间段 · 终点',
      windowHint: '每天只对这段时间内的请求计算补贴，不是整个自然日。终点同时也是自动执行的触发时刻。',
      invalidWindow: '时间段终点必须晚于起点',
      autoEnabled: '每天自动执行',
      autoEnabledHint: '打开后每天 {time} 自动运行这个任务（即上面填的时间段终点），不用手动点"运行"。',
      recurrenceType: '重复方式',
      recurrenceDaily: '每天',
      recurrenceWeekly: '每周',
      recurrenceWeekday: '工作日',
      weekdayMon: '周一',
      weekdayTue: '周二',
      weekdayWed: '周三',
      weekdayThu: '周四',
      ...
      weekdaySatShort: '六',
      weekdaySunShort: '日',
      invalidWeekdays: '选了"每周"就至少要勾一个星期几',
      autoBadgeWeekday: '自动 · 工作日 {time}',
      autoBadgeWeekly: '自动 · 每周{days} {time}',
      validityEnabled: '限定生效日期区间',
      validityHint: '不勾选 = 一直生效，跟上面的重复方式无关，只限定这条自动执行规则本身在哪段日期内起作用。',
      validFrom: '起始日期',
      validUntil: '结束日期',
      invalidValidity: '结束日期不能早于起始日期',
      validityFromOnly: '{date} 起生效',
      validityUntilOnly: '至 {date} 生效',
      confirmDelete: '确定删除？',
      from: '起始时间',
      to: '结束时间',
      thresholdSeconds: '慢请求阈值（秒）',
      ...
      requests: '请求数',
      compensation: '补贴金额',
      noneQualified: '这个时间段下没有符合条件、且尚未补贴过的请求。',
      invalidRange: '结束时间必须晚于起始时间',
      invalidThreshold: '阈值必须是大于 0 的整数',
      invalidRatio: '退款比例必须在 0% 到 100% 之间',
      previewFailed: '预览失败',
      applyFailed: '发放补贴失败',
      applyConfirmTitle: '确认发放补贴',
      applyConfirmMessage: '将给 {count} 位用户合计发放 ${amount} 补贴，直接加到 Sub2API 那边的用户余额上。确定继续吗？',
      applySuccess: '已给 {count} 位用户发放合计 ${amount} 补贴',
      confirm: '确认',
      cancel: '取消',
      historyTitle: '补贴发放历史',
      historyLoading: '历史记录加载中…',
      historyEmpty: '还没有发放过补贴。',
      historyUsersCount: '{count} 位用户',
      revoke: '撤回',
      confirmRevoke: '确定撤回整批？',
      revokedBadge: '已撤回',
      revokeSuccess: '已撤回，{count} 位用户的余额已扣回',
      revokePartialSuccess: '部分撤回：{revoked} 位用户已扣回，{skipped} 位用户余额不足未能扣回',
      revokeFailed: '撤回失败',
      autoSaveFailed: '保存失败'
    },
    groupRateCampaigns: {
    ...
    settings: {
      ...
      sections: {
        ...
        thresholds: {
          ...
          upstreamErrorWarning: '请求 502/503 错误预警',
          upstreamErrorWarningHelp: '轮询 Sub2API 最近请求错误，发现 502 或 503 时按分组发送飞书通知。',
          upstreamErrorCooldown: '同一工作区、同一分组 3 小时内只提醒一次。'
        },
        dailyReport: {
          title: '每日运营报告',
