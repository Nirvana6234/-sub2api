...
// ClaimUpstreamErrorAlert reserves one notification slot per workspace/group.
// The database-backed cooldown survives process restarts and concurrent ticks.
func (s *Service) ClaimUpstreamErrorAlert(ctx context.Context, userID, adminAccountID, groupKey string, cooldown time.Duration) (bool, error) {
	return s.repository.ClaimUpstreamErrorAlert(ctx, userID, adminAccountID, groupKey, cooldown)
}
...
// SendFormattedToFeishuBotsForWorkspace is the channel-restricted path used by
// alerts whose product contract explicitly says Feishu. A stale or crafted bot
// ID cannot redirect these alerts to another channel.
func (s *Service) SendFormattedToFeishuBotsForWorkspace(ctx context.Context, userID, adminAccountID string, botIDs []string, message string, format NotificationTemplateFormat) {
	if len(botIDs) == 0 || strings.TrimSpace(message) == "" {
		return
	}
	channels, err := s.repository.GetNotificationChannels(ctx, userID, adminAccountID)
	if err != nil {
		log.Printf("[settings] 读取飞书机器人失败 user_id=%s admin_account_id=%s err=%v", userID, adminAccountID, err)
		return
	}
	requested := make(map[string]struct{}, len(botIDs))
	for _, id := range botIDs {
		if id = strings.TrimSpace(id); id != "" {
			requested[id] = struct{}{}
		}
	}
	formatted := notificationMessage{Content: message, Format: normalizeNotificationTemplateFormat(format)}
	matched := 0
	for _, bot := range channels.Feishu {
		if _, ok := requested[bot.ID]; !ok || !bot.Enabled {
			continue
		}
		matched++
		if err := s.sendFeishuMessage(ctx, bot.Webhook, bot.Secret, formatted); err != nil {
			log.Printf("[settings] 飞书通知发送失败 bot=%s err=%v", bot.Name, err)
		}
	}
	if matched == 0 {
		log.Printf("[settings] 飞书通知机器人未匹配 user_id=%s admin_account_id=%s requested=%d", userID, adminAccountID, len(requested))
	}
}
