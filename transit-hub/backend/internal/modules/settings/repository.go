...
func (r *Repository) EnsureSchema(ctx context.Context) error {
	...
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS upstream_error_alert_cooldowns (
			user_id text NOT NULL,
			admin_account_id text NOT NULL,
			group_key text NOT NULL,
			last_notified_at timestamptz NOT NULL DEFAULT now(),
			cooldown_until timestamptz NOT NULL,
			PRIMARY KEY (user_id, admin_account_id, group_key)
		)`,
...
// ClaimUpstreamErrorAlert atomically claims a group alert after its cooldown.
func (r *Repository) ClaimUpstreamErrorAlert(ctx context.Context, userID, adminAccountID, groupKey string, cooldown time.Duration) (bool, error) {
	if cooldown <= 0 {
		cooldown = 3 * time.Hour
	}
	var claimed string
	err := r.db.QueryRow(ctx, `
		INSERT INTO upstream_error_alert_cooldowns (
			user_id, admin_account_id, group_key, last_notified_at, cooldown_until
		) VALUES ($1, $2, $3, now(), now() + ($4::bigint * interval '1 second'))
		ON CONFLICT (user_id, admin_account_id, group_key)
		DO UPDATE SET
			last_notified_at = now(),
			cooldown_until = now() + ($4::bigint * interval '1 second')
		WHERE upstream_error_alert_cooldowns.cooldown_until <= now()
		RETURNING group_key
	`, userID, adminAccountID, groupKey, int64(cooldown.Seconds())).Scan(&claimed)
