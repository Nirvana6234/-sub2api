...
type PlatformClient interface {
	...
	// FetchAdminUsageAccounting 同时返回营收与上游成本，两者同源同日界。
	FetchAdminUsageAccounting(session upstream.Session, startDate, endDate string) (upstream.AdminUsageAccounting, error)
...
func mapPlatformError(err error) requestError {
	...
	case upstream.ErrorAuth:
		return requestError(ErrorAuthRejected)
	case upstream.ErrorInteractiveLoginRequired:
		return requestError(ErrorInteractiveLoginRequired)
	case upstream.ErrorInvalidResponse:
		return requestError(ErrorInvalidResponse)
	case upstream.ErrorRequest:
		return requestError(ErrorUpstreamRequest)
	default:
