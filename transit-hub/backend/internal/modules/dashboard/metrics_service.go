...
//
// Calculation sources:
//   - todayProfitUSD: admin site's total actual usage for today (USD).
//   - siteBalanceUSD: filtered sum of admin-site user balances (USD).
//   - todayPurchaseCNY: admin site's account cost aggregate (CNY).
//   - upstreamBalanceCNY: cached upstream balance sum multiplied by site recharge rates (CNY).
//   - costStatus: admin_accounted only; missing authoritative cost fails the request.
//
// purchaseResult contains the admin platform's authoritative cost amount and status.
type purchaseResult struct {
	TotalCNY float64
	Status   CostStatus
}

// resolvePurchase only accepts the admin platform's account-cost aggregate.
// A missing total_account_cost is an unavailable cost, never permission to
// reconstruct it from TransitHub's cached sites or recharge rates.
// amountToCNYRate returns the coefficient needed to convert the platform's
// reporting amount to CNY.
//
// 配置里的 usdToCNYRate 只对真正以美元计价的平台成立。sub2api 返回的营收、
// 站点余额、账号成本本身就是人民币，再乘一次汇率会把整套账放大 7 倍——生产
// 8/9 那条 ¥2866.96 的成本就是 ¥409.57 被乘了 7，周期营收 ¥2174.96 同样是
// ¥310.49 被乘了 7。折算系数只能由金额的原始币种决定，不能由配置决定。
func amountToCNYRate(platform upstream.Platform, usdToCNYRate float64) float64 {
	if platform.ReportingCurrency() == upstream.AmountCurrencyUSD {
		return usdToCNYRate
	}
	return 1
}

func resolvePurchase(accounting upstream.AdminUsageAccounting, amountRate float64) purchaseResult {
	if !accounting.HasAccountCost {
		return purchaseResult{Status: CostStatusMissing}
	}
	return purchaseResult{
		TotalCNY: accounting.AccountCostUSD * amountRate,
		Status:   CostStatusAdminAccounted,
	}
}
...
func (s *MetricsService) LiveMetrics(ctx context.Context, userID string) (MetricsResponse, error) {
	...
	// 汇率与余额筛选同源：都存在 dashboard_balance_filter（每工作区一行）。
	// 这里同步取一次，goroutine 2 复用，避免重复查询。
	...
	amountRate := amountToCNYRate(session.Platform, filterConfig.EffectiveUSDToCNYRate())

	// goroutine 1: 今日营收与上游成本（平台中性，USD）。一次调用取回两个数字，
	// 保证它们来自同一段 usage_logs、同一日界。
	var adminAccounting upstream.AdminUsageAccounting
	var adminAccountingErr error
	wg.Add(1)
	go func() {
		defer wg.Done()
		accounting, err := s.platform.FetchAdminUsageAccounting(session, today, today)
		if err != nil {
			adminAccountingErr = err
			log.Printf("dashboard metrics: fetch usage stats failed user_id=%s err=%v", userID, err)
			return
		}
		adminAccounting = accounting
		todayProfitUSD = accounting.RevenueUSD
	}()

	// goroutine 2: 站点用户总余额（平台中性，USD）。
	wg.Add(1)
	go func() {
		defer wg.Done()
		filterConfig, err := s.metricsRepo.GetBalanceFilter(ctx, userID, adminAccountID)
		...
		siteBalanceUSD = balanceResult.Balance
	}()
	...
	// goroutine 4: 上游总余额（读取 Redis 缓存，CNY）。
	...
	go func() {
		...
		for _, site := range s.upstreams.ListForAccount(ctx, userID, adminAccountID) {
			...
			if site.Metrics.Balance.Value != nil {
				upstreamBalanceCNY += *site.Metrics.Balance.Value * site.RechargeRate
			}
		}
	}()
	...
	if adminAccountingErr != nil {
		return MetricsResponse{}, adminAccountingErr
	}
	purchaseRes := resolvePurchase(adminAccounting, amountRate)
	if purchaseRes.Status == CostStatusMissing {
		return MetricsResponse{}, errors.New("admin account cost unavailable")
	}

	// 营收和站点余额统一到 CNY：只有原本以美元计价的平台才真的需要折算。
	todayProfitCNY := todayProfitUSD * amountRate
	siteBalanceCNY := siteBalanceUSD * amountRate

	result := MetricsResponse{
		TodayProfitUSD:     FromFloat(CurrencyUSD, todayProfitUSD),
		SiteBalanceUSD:     FromFloat(CurrencyUSD, siteBalanceUSD),
		TodayProfitCNY:     FromFloat(CurrencyCNY, todayProfitCNY),
		SiteBalanceCNY:     FromFloat(CurrencyCNY, siteBalanceCNY),
		TodayPurchaseCNY:   FromFloat(CurrencyCNY, purchaseRes.TotalCNY),
		UpstreamBalanceCNY: FromFloat(CurrencyCNY, upstreamBalanceCNY),
		NetProfitCNY:       FromFloat(CurrencyCNY, todayProfitCNY-purchaseRes.TotalCNY),
		CostStatus:         purchaseRes.Status,
		USDToCNYRate:       amountRate,
...
func (s *MetricsService) Trends(ctx context.Context, userID string, days int) (TrendResponse, error) {
	...
	return TrendResponse{Points: authoritativeTrendPoints(snapshots)}, nil
}

// authoritativeTrendPoints excludes snapshots produced by the legacy local
// cost reconstruction path. Revenue, cost, and net profit are only meaningful
// when the row was written from Sub2API's authoritative accounting response.
func authoritativeTrendPoints(snapshots []DailySnapshot) []TrendPoint {
	points := make([]TrendPoint, 0, len(snapshots))
	for _, snap := range snapshots {
		if snap.CostStatus != CostStatusAdminAccounted {
			continue
		}
		// 用每行自己写入时的汇率，不用当前全局汇率。
		// 否则改一次汇率会追溯改写整条历史曲线（历史快照必须保留原值）。
		// 迁移前的旧行 usd_to_cny_rate 为 0，EffectiveRate() 兜底成默认值。
		rowRate := snap.EffectiveRate()
		todayProfitCNY := snap.TodayProfitUSD * rowRate
		siteBalanceCNY := snap.SiteBalanceUSD * rowRate
		netProfitCNY := todayProfitCNY - snap.TodayPurchaseCNY

		points = append(points, TrendPoint{
			Date:               snap.Date.Format("2006-01-02"),
			TodayProfitUSD:     FromFloat(CurrencyUSD, snap.TodayProfitUSD),
			TodayProfitCNY:     FromFloat(CurrencyCNY, todayProfitCNY),
			SiteBalanceUSD:     FromFloat(CurrencyUSD, snap.SiteBalanceUSD),
			SiteBalanceCNY:     FromFloat(CurrencyCNY, siteBalanceCNY),
			TodayPurchaseCNY:   FromFloat(CurrencyCNY, snap.TodayPurchaseCNY),
			NetProfitCNY:       FromFloat(CurrencyCNY, netProfitCNY),
			UpstreamBalanceCNY: FromFloat(CurrencyCNY, snap.UpstreamBalanceCNY),
...
func (s *MetricsService) finalizeSnapshot(ctx context.Context, userID, adminAccountID, date string) error {
	...
	accounting, err := s.platform.FetchAdminUsageAccounting(session, date, date)
	if err != nil {
		return err
	}
	profit := accounting.RevenueUSD
	filterCfg, err := s.metricsRepo.GetBalanceFilter(ctx, userID, adminAccountID)
	if err != nil {
		return err
	}
	amountRate := amountToCNYRate(session.Platform, filterCfg.EffectiveUSDToCNYRate())
	purchaseRes := resolvePurchase(accounting, amountRate)
	if purchaseRes.Status == CostStatusMissing {
		return errors.New("admin account cost unavailable")
	}
	...
	var upstreamBalanceCNY float64
	for _, site := range s.upstreams.ListForAccount(ctx, userID, adminAccountID) {
		if site.RechargeRate > 0 && site.Metrics.Balance.Value != nil {
			upstreamBalanceCNY += *site.Metrics.Balance.Value * site.RechargeRate
		}
	}
	return s.upsertSnapshot(ctx, userID, adminAccountID, date, MetricsResponse{
		TodayProfitUSD:     FromFloat(CurrencyUSD, profit),
		SiteBalanceUSD:     FromFloat(CurrencyUSD, balance.Balance),
		TodayPurchaseCNY:   FromFloat(CurrencyCNY, purchaseRes.TotalCNY),
		UpstreamBalanceCNY: FromFloat(CurrencyCNY, upstreamBalanceCNY),
		CostStatus:         purchaseRes.Status,
		// 补历史时也记下当时实际用的折算系数，历史行不受后续改汇率影响。
		USDToCNYRate: amountRate,
	}, true)
}
...
func (s *MetricsService) upsertSnapshot(ctx context.Context, userID, adminAccountID, date string, metrics MetricsResponse, finalized bool) error {
	...
	snapshot := DailySnapshot{
		...
		Date:               parsedDate,
		TodayProfitUSD:     metrics.TodayProfitUSD.Amount,
		SiteBalanceUSD:     metrics.SiteBalanceUSD.Amount,
		TodayPurchaseCNY:   metrics.TodayPurchaseCNY.Amount,
		UpstreamBalanceCNY: metrics.UpstreamBalanceCNY.Amount,
		CostStatus:         metrics.CostStatus,
		// 持久化写入当天所用的汇率，使历史行可复现。
		// 不存的话 Trends 只能拿当前汇率去乘历史 USD 值，改一次汇率整条历史曲线会被追溯改写。
		USDToCNYRate: metrics.USDToCNYRate,
		CreatedAt:    time.Now(),
