...
type fakePlatformClient struct {
	...
	// adminAccounting / adminAccountingErr 控制 FetchAdminUsageAccounting 的返回，
	// 用于验证成本口径在「admin 站点提供账号成本」与「回退上游采集」之间的切换。
	adminAccounting    upstream.AdminUsageAccounting
	adminAccountingErr error
...
// 默认不提供账号成本口径，等价于旧版 sub2api / new-api：成本仍走上游站点采集，
// 因此既有用例的期望值不变。需要验证新口径的用例自行覆盖 adminAccounting。
func (f *fakePlatformClient) FetchAdminUsageAccounting(session upstream.Session, startDate, endDate string) (upstream.AdminUsageAccounting, error) {
	return f.adminAccounting, f.adminAccountingErr
}
