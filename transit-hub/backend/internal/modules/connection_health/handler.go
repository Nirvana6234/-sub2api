...
func RegisterRoutes(mux *http.ServeMux, service *Service) {
	...
	mux.HandleFunc("GET /api/connection-health/targets/{id}/upstream-key-multiplier-history", handler.upstreamKeyMultiplierHistory)
	...
	mux.HandleFunc("PUT /api/connection-health/targets/{id}/manual-upstream-key-multiplier", handler.setManualUpstreamKeyMultiplier)
	mux.HandleFunc("DELETE /api/connection-health/targets/{id}/manual-upstream-key-multiplier", handler.clearManualUpstreamKeyMultiplier)
	...
	mux.HandleFunc("PUT /api/connection-health/admin-groups/{id}/status", handler.putAdminGroupStatus)
}

func (h *Handler) upstreamKeyMultiplierHistory(w http.ResponseWriter, r *http.Request) {
	userID, ok := authctx.UserID(r.Context())
	if !ok {
		httpjson.WriteError(w, http.StatusUnauthorized, "auth.errors.unauthorized")
		return
	}
	interval := r.URL.Query().Get("range")
	if interval != "day" && interval != "week" {
		interval = "hour"
	}
	points, err := h.service.UpstreamKeyMultiplierHistory(r.Context(), userID, r.PathValue("id"), interval)
	if err != nil {
		writeError(w, err)
		return
	}
	if points == nil {
		points = []UpstreamKeyMultiplierHistoryPoint{}
	}
	httpjson.Write(w, http.StatusOK, points)
}
...
func (h *Handler) setManualUpstreamKeyMultiplier(w http.ResponseWriter, r *http.Request) {
	userID, ok := authctx.UserID(r.Context())
	if !ok {
		httpjson.WriteError(w, http.StatusUnauthorized, "auth.errors.unauthorized")
		return
	}
	var input ManualUpstreamKeyMultiplierInput
	if err := httpjson.Decode(r, &input); err != nil {
		httpjson.WriteError(w, http.StatusBadRequest, ErrorRequest)
		return
	}
	value, err := h.service.SetManualUpstreamKeyMultiplier(r.Context(), userID, r.PathValue("id"), input)
	if err != nil {
		writeError(w, err)
		return
	}
...
func (h *Handler) clearManualUpstreamKeyMultiplier(w http.ResponseWriter, r *http.Request) {
	userID, ok := authctx.UserID(r.Context())
	if !ok {
		httpjson.WriteError(w, http.StatusUnauthorized, "auth.errors.unauthorized")
		return
	}
	if err := h.service.ClearManualUpstreamKeyMultiplier(r.Context(), userID, r.PathValue("id")); err != nil {
		writeError(w, err)
		return
	}
	httpjson.Write(w, http.StatusOK, map[string]bool{"ok": true})
...
func (h *Handler) putAdminGroupStatus(w http.ResponseWriter, r *http.Request) {
	userID, ok := authctx.UserID(r.Context())
	if !ok {
		httpjson.WriteError(w, http.StatusUnauthorized, "auth.errors.unauthorized")
		return
	...
	if err := httpjson.Decode(r, &input); err != nil || input.Active == nil {
		httpjson.WriteError(w, http.StatusBadRequest, ErrorRequest)
		return
	}
	if err := h.service.SetAdminGroupStatus(r.Context(), userID, r.PathValue("id"), *input.Active); err != nil {
		writeError(w, err)
		return
	}
	httpjson.Write(w, http.StatusOK, map[string]any{"active": *input.Active})
}
