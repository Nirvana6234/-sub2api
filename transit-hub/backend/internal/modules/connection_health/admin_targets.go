...
type AdminProbeTarget struct {
	...
	SchedulabilitySource    string     `json:"schedulabilitySource,omitempty"`
	SchedulabilityReason    string     `json:"schedulabilityReason,omitempty"`
	SchedulabilityChangedAt *time.Time `json:"schedulabilityChangedAt,omitempty"`
	RateLimitResetAt        *time.Time `json:"rateLimitResetAt,omitempty"`
	OverloadUntil           *time.Time `json:"overloadUntil,omitempty"`
	TempUnschedulableUntil  *time.Time `json:"tempUnschedulableUntil,omitempty"`
	TempUnschedulableReason string     `json:"tempUnschedulableReason,omitempty"`
	ExpiresAt               *time.Time `json:"expiresAt,omitempty"`
	AutoPauseOnExpired      *bool      `json:"autoPauseOnExpired,omitempty"`
...
func (s *Service) findAdminTarget(ctx context.Context, session upstream.Session, adminAccountID string, accountID string) (target AdminProbeTarget, account upstream.AdminGroupAccountInfo, found bool, accountsReadError bool, err error) {
	...
	for _, group := range groups {
		...
		for _, acc := range accounts {
			...
			applySub2APIRuntimeState(&resolved, acc)
...
func (s *Service) probeTargetOnce(ctx context.Context, userID string, adminAccountID string, target AdminProbeTarget, cred upstream.ProbeCredential, spec probeModelSpec) (*targetProbeResult, error) {
	...
	allowed, err := s.reserveProbeBudget(ctx, userID, adminAccountID, spec.policy, current.State, targetRequiresRecoveryProbe(target))
	if err != nil {
		return nil, err
	}
	if !allowed {
	...
	outcome := s.probeRunner.Probe(ctx, ProbeRequest{
		...
		AccountPlatform: cred.AccountPlatform, AccountType: cred.AccountType, Extra: cred.Extra, HeaderOverrides: cred.HeaderOverrides,
...
// Non-healthy targets must keep a low-frequency recovery path even after the
// normal daily probe budget is exhausted. Cooldown and ProbeBackoff still gate
// scheduled retries; only the daily cap is bypassed. Legacy transitional states
// are included so existing rows can converge to the binary health model.
func recoveryProbeBypassesDailyBudget(state State) bool {
	return state != StateHealthy && state != StateDisabled
}

func (s *Service) reserveProbeBudget(
	ctx context.Context,
	userID string,
	adminAccountID string,
	policy Policy,
	...
	if forceRecovery || recoveryProbeBypassesDailyBudget(state) {
		return true, nil
	}
	return s.repo.TryConsumeProbeBudget(
		ctx, userID, adminAccountID, policy.ID, probeBudgetDayStart(time.Now()), probeBudgetLimit(policy),
	)
}

// Sub2API 权威停用来源取值。这是区分「管理员手动关闭」与「系统自动停用」的唯一依据，
// 禁止再用 schedulable + status 的组合推断来源。
const (
...
// 恢复被拒的可审计原因码。
const (
	recoverySkipNotSub2API       = "not_sub2api"
	recoverySkipSourceMissing    = "schedulability_source_missing"
	recoverySkipSourceUnknown    = "schedulability_source_unknown"
	recoverySkipSourceManual     = "schedulability_source_manual"
	recoverySkipSourceNone       = "schedulability_source_none"
	recoverySkipStatusNotError   = "status_not_error"
	recoverySkipRuntimeWindow    = "runtime_block_window_active"
	recoverySkipExpiredAutoPause = "expired_auto_paused"
)

func targetRequiresRecoveryProbe(target AdminProbeTarget) bool {
	return sub2APIErrorRecoveryEligible(target, time.Now())
}

func sub2APIErrorRecoveryEligible(target AdminProbeTarget, now time.Time) bool {
	eligible, _ := sub2APIErrorRecoveryDecision(target, now)
	return eligible
}

// sub2APIErrorRecoveryDecision 只消费 Sub2API 给出的明确来源。
// 来源缺失、未知或非法时一律失败关闭：不探测、不恢复、不猜测，并返回可审计原因。
func sub2APIErrorRecoveryDecision(target AdminProbeTarget, now time.Time) (bool, string) {
	if target.Platform != string(upstream.PlatformSub2API) {
		return false, recoverySkipNotSub2API
	}
	switch source := strings.ToLower(strings.TrimSpace(target.SchedulabilitySource)); source {
	case SchedulabilitySourceAutomatic:
		// 系统自动停用，允许进入定期真实生成恢复检查。
		// automatic 时即使 schedulable=false 也必须能被识别。
	case "":
		return false, recoverySkipSourceMissing
	case SchedulabilitySourceManual:
		return false, recoverySkipSourceManual
	case SchedulabilitySourceNone:
		return false, recoverySkipSourceNone
	default:
		return false, recoverySkipSourceUnknown
	}
	// 「系统自动停用且尚未恢复」的权威标志是 source=automatic（上面已确认）加上
	// 当前仍不可调度，而不是 status 停在 error。错误信息会被 403 冷却结束或管理员
	// 清错误抹掉，schedulable 却不会自己变回来，于是账号会落进一条缝隙：
	// status=active + schedulable=false 时，常规探活因运行时阻塞跳过它，恢复探测
	// 又因 status≠error 不认它，两个门都进不去，永远不会被探活、也永远不会自愈。
	// 这里把两种形态都收进来；正常可调度的账号仍然不进恢复路径。
	statusIsError := strings.EqualFold(strings.TrimSpace(target.AccountStatus), "error")
	notSchedulable := target.AccountSchedulable != nil && !*target.AccountSchedulable
	if !statusIsError && !notSchedulable {
		return false, recoverySkipStatusNotError
	}
	for _, until := range []*time.Time{target.RateLimitResetAt, target.OverloadUntil, target.TempUnschedulableUntil} {
		if until != nil && now.Before(*until) {
			return false, recoverySkipRuntimeWindow
		}
	}
	if target.AutoPauseOnExpired != nil && *target.AutoPauseOnExpired &&
		target.ExpiresAt != nil && !now.Before(*target.ExpiresAt) {
		return false, recoverySkipExpiredAutoPause
	}
	return true, ""
}

// isSub2APIManuallyDisabled 只认权威来源 manual。
// 刻意不看 schedulable：Sub2API 的系统自动隔离路径（凭据错误、OAuth 错误、
// Grok 隔离、过期自动暂停）同样会写 schedulable=false，按 schedulable 推断
// 会把 automatic 误判成 manual，使其永远无法自动恢复。
func isSub2APIManuallyDisabled(target AdminProbeTarget) bool {
	return target.Platform == string(upstream.PlatformSub2API) &&
		strings.EqualFold(strings.TrimSpace(target.SchedulabilitySource), SchedulabilitySourceManual)
}

func isSub2APIRuntimeBlocked(target AdminProbeTarget, now time.Time) bool {
	if target.Platform != string(upstream.PlatformSub2API) {
		return false
	}
	if isSub2APIManuallyDisabled(target) {
		return true
	}
	// schedulable=false 一律算运行时阻塞，与来源无关：不可调度的账号不该收到常规探测。
	// 这与 isSub2APIManuallyDisabled 是两件事——后者只回答「所有权是否归管理员、
	// 能否自动恢复」，这里回答「现在能不能走常规探测」。
	if target.AccountSchedulable != nil && !*target.AccountSchedulable {
		return true
	}
	status := strings.ToLower(strings.TrimSpace(target.AccountStatus))
	if status != "" && status != "active" && status != "enabled" && status != "1" {
		return true
	}
	for _, until := range []*time.Time{target.RateLimitResetAt, target.OverloadUntil, target.TempUnschedulableUntil} {
		if until != nil && now.Before(*until) {
	...
	return target.AutoPauseOnExpired != nil && *target.AutoPauseOnExpired &&
		target.ExpiresAt != nil && !now.Before(*target.ExpiresAt)
}

func applySub2APIRuntimeState(target *AdminProbeTarget, account upstream.AdminGroupAccountInfo) {
	target.AccountSchedulable = cloneBoolPointer(account.Schedulable)
	// 来源原样透传：空值/未知值不得在此被补成 none，否则就把「来源不明」
	// 悄悄变成了「正常账号」，等于用推断替代可靠标记。
	target.SchedulabilitySource = account.SchedulabilitySource
	target.SchedulabilityReason = account.SchedulabilityReason
	target.SchedulabilityChangedAt = cloneTimePointer(account.SchedulabilityChangedAt)
	target.RateLimitResetAt = cloneTimePointer(account.RateLimitResetAt)
	target.OverloadUntil = cloneTimePointer(account.OverloadUntil)
	target.TempUnschedulableUntil = cloneTimePointer(account.TempUnschedulableUntil)
	target.TempUnschedulableReason = account.TempUnschedulableReason
	target.ExpiresAt = cloneTimePointer(account.ExpiresAt)
	if account.AutoPauseOnExpired != nil {
		value := *account.AutoPauseOnExpired
		target.AutoPauseOnExpired = &value
	}
}
