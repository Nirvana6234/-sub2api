...
func (s *Service) DiscoverTargetModels(ctx context.Context, userID string, targetID string) ([]DiscoveredModel, error) {
	...
	if models := discoveredModelsFromKnownList(account.Models); len(models) > 0 {
		return models, nil
	}
...
func discoveredModelsFromKnownList(raw string) []DiscoveredModel {
	parts := splitModelList(raw)
	if len(parts) == 0 {
		return nil
	}
