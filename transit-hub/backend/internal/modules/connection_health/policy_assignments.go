	...
	for _, a := range assignments {
		...
		if p, ok := policyByID[a.PolicyID]; ok {
			summaries = append(summaries, AssignedPolicySummary{
				...
				PriorityMode: normalizePriorityMode(p.PriorityMode), PriorityStrategy: normalizePriorityStrategy(p.PriorityStrategy), AutoRemoteActionEnabled: policyRemoteActionEnabled(p),
			})
		} else {
