...
type healthRepository interface {
	...
	ListManualUpstreamKeyMultipliers(ctx context.Context, userID string, adminAccountID string) ([]ManualUpstreamKeyMultiplier, error)
	UpsertManualUpstreamKeyMultiplier(ctx context.Context, value ManualUpstreamKeyMultiplier) error
	DeleteManualUpstreamKeyMultiplier(ctx context.Context, userID string, adminAccountID string, targetID string) error
...
func NewService(repo *Repository, mySites MySitesReader, sites SiteLookup, platform PlatformActioner) *Service {
	...
	if statusWriter, ok := platform.(PlatformGroupStatusWriter); ok {
		service.groupStatus = statusWriter
	}
	if recoveryActions, ok := platform.(Sub2APIRecoveryActioner); ok {
		service.recoveryActions = recoveryActions
	}
	return service
}
...
type StoredSummaryResponse struct {
	...
	// UnmonitoredTargets 是只剩历史状态、当前已不被任何启用策略探活的目标数。
	// 它们不计入上面四项，否则面板会长期挂着一片无人会去处理的假红。
	UnmonitoredTargets  int        `json:"unmonitoredTargets"`
	ManagedTargets      int        `json:"managedTargets"`
	RecentFailureEvents int        `json:"recentFailureEvents"`
	LastProbeAt         *time.Time `json:"lastProbeAt,omitempty"`
}

// monitoredModelNames 收集当前 workspace 内仍会被真实探活的模型名。
//
// 判据与调度器的 candidateModelSpecs 保持一致：只有启用策略里启用的 ModelTarget
// 才会进入探活队列。策略把某个模型从清单里移除后，该模型的历史状态行不会再收到
// 任何探活结果，也就永远停在移除当天的状态——冷却早已到期却不会被重排，
// 状态机也没有输入可以推进它。这类行必须与真实告警分开统计。
//
// 精度限制：这里按 workspace 汇总模型名，不区分「某个 target 是否分配了包含该
// 模型的那条策略」。因此一个模型只要还被任意启用策略覆盖，未分配该策略的 target
// 上的同名状态行仍会被算作受监控。这是有意的保守取舍——宁可少标记，也不要把
// 真实告警误判成僵尸而藏起来。
func monitoredModelNames(policies []Policy) map[string]struct{} {
	names := make(map[string]struct{})
	for _, policy := range policies {
		if !policy.Enabled || !policySupportsProbing(policy) {
			continue
		}
		for _, target := range policy.ModelTargets {
			if !target.Enabled {
				continue
			}
			if name := strings.TrimSpace(target.ModelName); name != "" {
				names[name] = struct{}{}
			}
		}
	}
...
// stateIsMonitored 判断一条健康状态行当前是否仍在探活覆盖内。
// disabled 是管理员显式停用，属于有人负责的状态，不算僵尸。
func stateIsMonitored(state ConnectionHealthState, monitored map[string]struct{}) bool {
	if state.State == StateDisabled {
		return true
	}
	_, ok := monitored[strings.TrimSpace(state.ModelName)]
	return ok
}
...
func (s *Service) StoredSummary(ctx context.Context, userID string) (StoredSummaryResponse, error) {
	...
	monitored := monitoredModelNames(policies)
	...
	unmonitoredTargets := make(map[string]struct{})
	var lastProbeAt *time.Time
	for _, state := range states {
		if !stateIsMonitored(state, monitored) {
			// 已不在探活覆盖内：单独计数，不参与健康/告警/熔断的风险聚合。
			unmonitoredTargets[state.ConnectionID] = struct{}{}
			continue
		}
		risk := targetHealthy
		switch state.State {
	...
	// 同一个 target 可能既有在探模型又有历史遗留模型，此时它仍是一个真实目标，
	// 不能重复计进「已不监控」。
	for connectionID := range targetRisk {
		delete(unmonitoredTargets, connectionID)
	}

	response := StoredSummaryResponse{
		TotalTargets:        len(targetRisk),
		UnmonitoredTargets:  len(unmonitoredTargets),
...
func (s *Service) SavePolicy(ctx context.Context, userID string, in PolicyInput) (Policy, error) {
	...
	if id == "" {
		...
		// 旧版前端不会发送 probeMode。更新已有策略时保留原模式，避免回退到默认值。
...
func buildPolicyAndTargets(userID string, adminAccountID string, id string, in PolicyInput) (Policy, []ModelTarget, error) {
	...
	policy := Policy{
		...
		ProbeMode: normalizeProbeMode(in.ProbeMode), ProbeIntervalSeconds: defaultInt(in.ProbeIntervalSeconds, 60),
		...
		PriorityStrategy: normalizePriorityStrategy(in.PriorityStrategy),
		StrategyMode:     strategyMode,
		DailyProbeBudget: defaultInt(in.DailyProbeBudget, 1000),
	}
	if strategyMode == StrategyModeMultiplierOnly {
		// 仅倍率策略不拥有任何常规探活行为。即使错误或旧客户端同时提交了探活字段，也在服务端
		// 强制关闭并丢弃模型目标。Sub2API automatic 已关闭账号的专用恢复探测由调度器独立构造，
		// 不会让已打开账号进入模型候选、探活预算或健康状态机。
		policy.AutoDegradeEnabled = false
		policy.AutoRemoteActionEnabled = false
		policy.PriorityMode = PriorityModeMultiplier
		policy.PriorityStrategy = PriorityStrategyPrice
...
func normalizePriorityStrategy(strategy string) string {
	switch strings.ToLower(strings.TrimSpace(strategy)) {
	case PriorityStrategyBalanced:
		return PriorityStrategyBalanced
	case PriorityStrategySpeed:
		return PriorityStrategySpeed
	default:
...
func (s *Service) probeOnce(ctx context.Context, conn my_sites.RealConnection, policy Policy, target ModelTarget) (*ConnectionHealthState, error) {
	...
	allowed, err := s.reserveProbeBudget(ctx, policy.UserID, policy.AdminAccountID, policy, current.State, false)
	if err != nil {
		return nil, err
	}
	if !allowed {
...
// normalizeProbeMode 将前端传入的 probeMode 规范化为合法的后端常量。
// 空值或未知值统一退回 ProbeModeRealModel，保持旧策略行为不变。
func normalizeProbeMode(mode string) string {
	if strings.TrimSpace(mode) == ProbeModeModelsEndpoint {
		return ProbeModeModelsEndpoint
	}
	return ProbeModeRealModel
}
