...
type fakeRepository struct {
	...
	manualMultipliers  map[string]ManualUpstreamKeyMultiplier
	multiplierHistory  map[string][]UpstreamKeyMultiplierHistoryPoint
...
func (f *fakeRepository) ListUpstreamKeyMultiplierHistory(_ context.Context, userID, adminAccountID, targetID, _ string) ([]UpstreamKeyMultiplierHistoryPoint, error) {
	values := f.multiplierHistory[userID+"|"+adminAccountID+"|"+targetID]
	return append([]UpstreamKeyMultiplierHistoryPoint(nil), values...), nil
}

func (f *fakeRepository) EnsureSchema(ctx context.Context) error { return nil }

func (f *fakeRepository) ListManualUpstreamKeyMultipliers(ctx context.Context, userID string, adminAccountID string) ([]ManualUpstreamKeyMultiplier, error) {
	values := make([]ManualUpstreamKeyMultiplier, 0)
	for _, value := range f.manualMultipliers {
		if value.UserID == userID && value.AdminAccountID == adminAccountID {
...
func (f *fakeRepository) UpsertManualUpstreamKeyMultiplier(ctx context.Context, value ManualUpstreamKeyMultiplier) error {
	if f.manualMultipliers == nil {
		f.manualMultipliers = map[string]ManualUpstreamKeyMultiplier{}
	}
	f.manualMultipliers[value.UserID+"|"+value.AdminAccountID+"|"+value.TargetID] = value
	return nil
}

func (f *fakeRepository) DeleteManualUpstreamKeyMultiplier(ctx context.Context, userID string, adminAccountID string, targetID string) error {
	delete(f.manualMultipliers, userID+"|"+adminAccountID+"|"+targetID)
	return nil
}
...
func (f *fakeRepository) ReplaceGroupPolicyConfiguration(ctx context.Context, userID string, adminAccountID string, adminGroupID string, adminGroupName string, policyIDs []string, excludedTargetIDs []string, groupTargetIDs []string) error {
	...
	groupPrefix := adminGroupID + "|"
	groupTargets := make(map[string]struct{}, len(groupTargetIDs))
	for _, targetID := range groupTargetIDs {
		groupTargets[targetID] = struct{}{}
	}
	for key, state := range f.priorityStates {
		if !state.Conflict || !strings.HasPrefix(state.TargetID, groupPrefix) {
			continue
		}
		if _, ok := groupTargets[strings.TrimPrefix(state.TargetID, groupPrefix)]; ok {
			delete(f.priorityStates, key)
		}
	}
	return nil
...
func TestStoredSummary_DeduplicatesTargetsAndKeepsWorkspaceIsolated(t *testing.T) {
	...
	// 两个模型都仍在启用策略的探活清单里，汇总才应该把它们计入健康风险。
	repo.policies = []Policy{{
		ID: "policy-1", UserID: "user1", AdminAccountID: "ws1", Enabled: true, StrategyMode: StrategyModeHealthProbe,
		ModelTargets: []ModelTarget{
			{ID: "mt-a", PolicyID: "policy-1", ModelName: "model-a", Enabled: true},
			{ID: "mt-b", PolicyID: "policy-1", ModelName: "model-b", Enabled: true},
		},
	}}
...
// TestStoredSummary_ExcludesStatesNoLongerCoveredByAnyPolicy 回归一次真实的持续性
// 误报：策略把某个模型从探活清单里移除后，该模型的历史状态行不会再收到任何探活
// 结果，冷却早已到期也不会被重排，于是永远停在移除当天的 suspended。生产上因此
// 出现「20 个目标里 8 个 suspended」的红色面板，而真正在探的连接全是健康的。
func TestStoredSummary_ExcludesStatesNoLongerCoveredByAnyPolicy(t *testing.T) {
	now := time.Now()
	staleProbe := now.Add(-10 * 24 * time.Hour)
	freshProbe := now.Add(-time.Minute)
	repo := newFakeRepository()

	// 仍在探活清单里的模型：正常参与统计。
	repo.states["target-live"] = map[string]ConnectionHealthState{
		"gpt-5.5": {ConnectionID: "target-live", ModelName: "gpt-5.5", UserID: "user1", AdminAccountID: "ws1", State: StateHealthy, LastProbeAt: &freshProbe},
	}
	// 已被移出清单的模型，且该目标再无其他模型：整个目标只剩历史。
	repo.states["target-zombie"] = map[string]ConnectionHealthState{
		"gpt-5.6-sol": {ConnectionID: "target-zombie", ModelName: "gpt-5.6-sol", UserID: "user1", AdminAccountID: "ws1", State: StateSuspended, LastProbeAt: &staleProbe},
		"gpt5.4-mini": {ConnectionID: "target-zombie", ModelName: "gpt5.4-mini", UserID: "user1", AdminAccountID: "ws1", State: StateDegraded, LastProbeAt: &staleProbe},
	}
	// 混合目标：既有在探模型也有历史遗留模型，它仍是一个真实目标。
	repo.states["target-mixed"] = map[string]ConnectionHealthState{
		"gpt-5.5": {ConnectionID: "target-mixed", ModelName: "gpt-5.5", UserID: "user1", AdminAccountID: "ws1", State: StateHealthy},
		"gpt-5.4": {ConnectionID: "target-mixed", ModelName: "gpt-5.4", UserID: "user1", AdminAccountID: "ws1", State: StateSuspended, LastProbeAt: &staleProbe},
	}
	repo.policies = []Policy{{
		ID: "policy-1", UserID: "user1", AdminAccountID: "ws1", Enabled: true, StrategyMode: StrategyModeHealthProbe,
		ModelTargets: []ModelTarget{{ID: "mt-1", PolicyID: "policy-1", ModelName: "gpt-5.5", Enabled: true}},
	}}
	service := &Service{repo: repo, accounts: fakeAdminAccountResolver{id: "ws1"}}
	...
	if summary.SuspendedTargets != 0 {
		t.Fatalf("stale states must not be reported as suspended: %+v", summary)
	}
	if summary.AttentionTargets != 0 {
		t.Fatalf("stale states must not be reported as attention: %+v", summary)
	}
	if summary.HealthyTargets != 2 || summary.TotalTargets != 2 {
		t.Fatalf("expected the two live targets to remain counted: %+v", summary)
	}
	if summary.UnmonitoredTargets != 1 {
		t.Fatalf("expected exactly the zombie-only target to be reported unmonitored: %+v", summary)
	}
	// 陈旧行不该把「最近探活时间」拖回 10 天前。
	if summary.LastProbeAt == nil || !summary.LastProbeAt.Equal(freshProbe) {
		t.Fatalf("expected last probe %v, got %v", freshProbe, summary.LastProbeAt)
	}
}

// 管理员显式停用是有人负责的状态，不能被当成僵尸藏起来。
func TestStoredSummary_DisabledStatesStayVisibleEvenWhenModelUnlisted(t *testing.T) {
	repo := newFakeRepository()
	repo.states["target-disabled"] = map[string]ConnectionHealthState{
		"retired-model": {ConnectionID: "target-disabled", ModelName: "retired-model", UserID: "user1", AdminAccountID: "ws1", State: StateDisabled},
	}
	repo.policies = []Policy{{
		ID: "policy-1", UserID: "user1", AdminAccountID: "ws1", Enabled: true, StrategyMode: StrategyModeHealthProbe,
		ModelTargets: []ModelTarget{{ID: "mt-1", PolicyID: "policy-1", ModelName: "gpt-5.5", Enabled: true}},
	...
	if summary.SuspendedTargets != 1 || summary.UnmonitoredTargets != 0 {
		t.Fatalf("manually disabled targets must stay visible: %+v", summary)
	}
}
