	...
	for _, assignment := range assignments {
		...
		if policy, ok := policyByID[assignment.PolicyID]; ok {
			result.Policies = append(result.Policies, AssignedPolicySummary{
				...
				PriorityMode: normalizePriorityMode(policy.PriorityMode), PriorityStrategy: normalizePriorityStrategy(policy.PriorityStrategy), AutoRemoteActionEnabled: policyRemoteActionEnabled(policy),
			})
		} else {
