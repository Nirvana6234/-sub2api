...
func (s *Service) reconcileTargetRemoteAction(
	...
	if target.Platform == string(upstream.PlatformSub2API) {
		// Sub2API exposes permanent administrator intent and temporary runtime
		// blocking separately. Health states still influence priority, but remote
		// actions must never overwrite the administrator's schedulable switch.
		if stored != nil {
			if err := s.repo.DeleteTargetActionState(ctx, userID, adminAccountID, target.TargetID); err != nil {
				return "", err
			}
		}
	...
	if target.Platform == string(upstream.PlatformSub2API) &&
		normalizeTargetStatus(target.Platform, target.AccountStatus) == "inactive" {
		return RemoteActionSkippedTargetInitiallyDisabled, nil
	}
	// 普通 degraded 只记录模型健康；只有已经接管或进入暂停/观察/恢复阶段时才修改上游。
	if stored == nil && (!statesComplete || (!blocked && !hasRecoveringState(states) && !targetRequiresRecoveryProbe(target))) {
		return "", nil
	}
	...
	currentStatus := targetControlStatus(target)
	...
	if stored == nil {
		...
		if target.Platform != string(upstream.PlatformSub2API) && !targetStatusEnabled(target.Platform, currentStatus) {
...
func (s *Service) restoreUnmanagedTargetActions(
	...
	for _, stored := range states {
		if parsed, ok := parseTargetID(stored.TargetID); ok && parsed.platform == string(upstream.PlatformSub2API) {
			// These snapshots belong to the removed implementation that wrote the
			// permanent schedulable switch. Clear only local ownership metadata; do
			// not infer that the current upstream value may be enabled.
			if err := s.repo.DeleteTargetActionState(ctx, stored.UserID, stored.AdminAccountID, stored.TargetID); err != nil {
				log.Printf("[connection-health] clear legacy Sub2API target action state failed target_id=%s err=%v", stored.TargetID, err)
			}
			continue
		}
			...
			for _, account := range groupInventory.accounts {
				...
				if !found {
					target = AdminProbeTarget{
						...
						AccountSchedulable: cloneBoolPointer(account.Schedulable),
		...
		if !found {
			...
			if parsed.platform == string(upstream.PlatformSub2API) {
				...
				currentlySchedulable := normalizeTargetStatus(parsed.platform, stored.LastAppliedStatus) != "inactive"
				target.AccountSchedulable = &currentlySchedulable
			}
		}
		currentStatus := targetControlStatus(target)
		currentWeight := normalizedTargetWeight(target)
		if targetVisible && target.Platform == string(upstream.PlatformSub2API) &&
			normalizeTargetStatus(target.Platform, target.AccountStatus) == "inactive" {
			stored.Conflict = true
			stored.PendingStatus = ""
			stored.PendingWeight = nil
			if err := s.repo.UpsertTargetActionState(ctx, stored); err != nil {
				log.Printf("[connection-health] protect manually disabled target failed target_id=%s err=%v", stored.TargetID, err)
			}
			continue
		}
...
func targetControlStatus(target AdminProbeTarget) string {
	if target.Platform == string(upstream.PlatformSub2API) {
