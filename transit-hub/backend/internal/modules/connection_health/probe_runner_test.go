...
func TestProbe_CompletedGenerationRequirementRejectsCreatedOnly(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("data: {\"type\":\"response.created\",\"response\":{\"status\":\"in_progress\"}}\n\n"))
	...
	outcome := NewRealProbeRunner().Probe(context.Background(), ProbeRequest{
		BaseURL: server.URL, UpstreamKey: "api-key", ProviderFamily: ProviderOpenAI,
		AccountPlatform: "openai", AccountType: "apikey", ModelName: "gpt-5.6-sol",
		RequireCompletedGeneration: true,
	})
	if outcome.Result != ResultInvalidResponse {
		t.Fatalf("created acknowledgement must not prove recovery, got %s", outcome.Result)
	}
}

func TestProbe_CompletedGenerationRequirementAcceptsCompletedEvent(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("data: {\"type\":\"response.completed\"}\n\n"))
	...
	if outcome.Result != ResultOK {
		t.Fatalf("completed response must prove recovery, got %s (%s)", outcome.Result, outcome.Detail)
	}
}
