...
type groupStatusUpdateCall struct {
	groupID string
	active  bool
}

type fakePlatformGroupStatusWriter struct {
	calls []groupStatusUpdateCall
}

func (f *fakePlatformGroupStatusWriter) UpdateAdminGroupStatus(_ upstream.Session, groupID string, active bool) error {
	f.calls = append(f.calls, groupStatusUpdateCall{groupID: groupID, active: active})
	return nil
}
...
func TestSetAdminGroupStatus_VerifiesGroupBelongsToCurrentWorkspace(t *testing.T) {
	repo := newFakeRepository()
	reader := fakePlatformGroupReader{groups: []upstream.AdminGroupInfo{{ID: "77", Name: "vip"}}}
	writer := &fakePlatformGroupStatusWriter{}
	service := newAdminGroupsService(
		reader,
		fakeMySitesReader{session: upstream.Session{Platform: upstream.PlatformSub2API}},
		repo,
	)
	service.groupStatus = writer

	if err := service.SetAdminGroupStatus(context.Background(), "user1", "77", false); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(writer.calls) != 1 || writer.calls[0] != (groupStatusUpdateCall{groupID: "77", active: false}) {
		t.Fatalf("unexpected status writes: %+v", writer.calls)
	}

	if err := service.SetAdminGroupStatus(context.Background(), "user1", "other-workspace-group", true); err == nil {
		t.Fatal("unknown group must not be sent to the upstream status writer")
	}
	if len(writer.calls) != 1 {
		t.Fatalf("unknown group must not produce an upstream write: %+v", writer.calls)
	}
}
...
type failingUpstreamKeyGroupSnapshotReader struct {
	fakeMySitesReader
}

func (f failingUpstreamKeyGroupSnapshotReader) ListUpstreamKeyGroupSnapshotsForWorkspace(context.Context, string, string, string) ([]my_sites.UpstreamKeyGroupSnapshot, error) {
	return nil, errors.New("upstream read unavailable")
}
...
func TestAdminGroups_UsesLatestUpstreamKeyMultiplierHistory(t *testing.T) {
	forwardingAccountMultiplier := 1.75
	unlinkedAccountMultiplier := 2.25
	upstreamKeyGroupMultiplier := 0.42
	manualMultiplier := 0.08
	repo := newFakeRepository()
	repo.manualMultipliers["user1|ws1|sub2api:ws1:100"] = ManualUpstreamKeyMultiplier{
		UserID: "user1", AdminAccountID: "ws1", TargetID: "sub2api:ws1:100", Multiplier: manualMultiplier,
	}
	repo.multiplierHistory["user1|ws1|sub2api:ws1:100"] = []UpstreamKeyMultiplierHistoryPoint{{
		ObservedAt: time.Date(2026, 8, 7, 12, 0, 0, 0, time.UTC), Multiplier: upstreamKeyGroupMultiplier, Source: "detected",
	}}
	...
	if linked.UpstreamKeyGroupName != "historical-vip" || linked.UpstreamKeyGroupMultiplier == nil {
		t.Fatalf("expected latest upstream API key history, got %+v", linked)
	}
	...
	if linked.UpstreamKeyGroupMultiplierSource != "detected" {
		t.Fatalf("detected upstream API key multiplier must declare its source, got %q", linked.UpstreamKeyGroupMultiplierSource)
	}
...
func TestAdminGroupsDoesNotUseSiteCacheWhenHistoryIsUnavailable(t *testing.T) {
	cachedMultiplier := 0.07
	mySites := failingUpstreamKeyGroupSnapshotReader{fakeMySitesReader: fakeMySitesReader{
		session: upstream.Session{Platform: upstream.PlatformSub2API},
		connections: []my_sites.RealConnection{{
			UserID:                  "user1",
			WorkspaceAdminAccountID: "ws1",
			UpstreamSiteID:          "site-1",
			UpstreamGroupID:         "group-7",
			UpstreamGroupName:       "upstream-vip",
			UpstreamKeyID:           "key-7",
			AdminAccountID:          "100",
			AdminPlatform:           string(upstream.PlatformSub2API),
			Status:                  my_sites.ConnectionStatusActive,
	...
	svc.sites = fakeSiteLookup{site: &upstream.Site{ID: "site-1", Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{
		ID: "group-7", Name: "upstream-vip", Multiplier: &cachedMultiplier,
	}}}}}

	groups, err := svc.AdminGroups(context.Background(), "user1")
	if err != nil {
		t.Fatalf("AdminGroups returned error: %v", err)
	}
	account := groups[0].Accounts[0]
	if account.UpstreamKeyGroupMultiplier != nil || account.UpstreamKeyGroupMultiplierSource != "" {
		t.Fatalf("health page must not use a separate site cache, got %+v", account)
	}
}

func TestAdminGroupsUsesLatestDetectedHistoryBeforeSiteCache(t *testing.T) {
	oldSiteMultiplier := 0.09
	historyMultiplier := 0.05
	repo := newFakeRepository()
	repo.multiplierHistory["user1|ws1|sub2api:ws1:100"] = []UpstreamKeyMultiplierHistoryPoint{
		{ObservedAt: time.Date(2026, 8, 7, 12, 0, 0, 0, time.UTC), Multiplier: historyMultiplier, Source: "detected"},
	}
	mySites := failingUpstreamKeyGroupSnapshotReader{fakeMySitesReader: fakeMySitesReader{
		session: upstream.Session{Platform: upstream.PlatformSub2API},
		connections: []my_sites.RealConnection{{
	...
	reader := fakePlatformGroupReader{
		groups:        []upstream.AdminGroupInfo{{ID: "own-group", Name: "own-vip"}},
		accountsByGrp: map[string][]upstream.AdminGroupAccountInfo{"own-group": {{ID: "100", Name: "linked"}}},
	}
	svc := newAdminGroupsService(reader, mySites, repo)
	svc.sites = fakeSiteLookup{site: &upstream.Site{ID: "site-1", Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{
		ID: "group-7", Name: "upstream-vip", Multiplier: &oldSiteMultiplier,
	}}}}}
	...
	account := groups[0].Accounts[0]
	if account.UpstreamKeyGroupMultiplier == nil || *account.UpstreamKeyGroupMultiplier != historyMultiplier {
		t.Fatalf("expected latest history multiplier, got %+v", account)
	}
	if account.UpstreamKeyGroupMultiplierSource != "detected" {
		t.Fatalf("history detected source = %q, want detected", account.UpstreamKeyGroupMultiplierSource)
	}
}

func TestAdminGroups_UsesManualUpstreamAPIKeyMultiplierOnlyWhenDetectionUnavailable(t *testing.T) {
	manualMultiplier := 0.08
	repo := newFakeRepository()
	repo.manualMultipliers["user1|ws1|newapi:ws1:100"] = ManualUpstreamKeyMultiplier{
		UserID: "user1", AdminAccountID: "ws1", TargetID: "newapi:ws1:100", Multiplier: manualMultiplier,
	}
	reader := fakePlatformGroupReader{
		groups: []upstream.AdminGroupInfo{{ID: "g1", Name: "vip"}},
		accountsByGrp: map[string][]upstream.AdminGroupAccountInfo{
			"g1": {{ID: "100", Name: "unlinked", BaseURL: "https://up", Models: "gpt-4o"}},
		},
	}
	...
	account := groups[0].Accounts[0]
	if account.UpstreamKeyGroupMultiplier == nil || *account.UpstreamKeyGroupMultiplier != manualMultiplier {
		t.Fatalf("manual multiplier was not used as fallback: %+v", account)
	}
	if account.UpstreamKeyGroupMultiplierSource != "manual" {
		t.Fatalf("manual multiplier source = %q, want manual", account.UpstreamKeyGroupMultiplierSource)
	}
	if account.UpstreamKeyGroupName != "" {
		t.Fatalf("manual fallback must not invent an upstream group name, got %q", account.UpstreamKeyGroupName)
	}
}
