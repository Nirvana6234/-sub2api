...
// PriorityMode 控制策略是否同步上游账号/渠道的调度优先级。none 保持上游设置；
// multiplier 仅按分组倍率排序；auto 使用 Sub2API Auto 的可观测评分逻辑。
const (
	PriorityModeNone       = "none"
	PriorityModeMultiplier = "multiplier"
	PriorityModeAuto       = "auto"
)

// PriorityStrategy mirrors Sub2API Auto Group selection. Price uses cost only,
// balanced combines 70% cost with 30% speed, and speed uses latency only.
const (
	PriorityStrategyPrice    = "price"
	PriorityStrategyBalanced = "balanced"
	PriorityStrategySpeed    = "speed"
)

// StrategyMode 明确区分策略是否需要常规真实模型探活。旧策略和旧客户端缺省为 health_probe；
// multiplier_only 只读取 admin 分组倍率并同步优先级，不对已打开账号做常规探活。唯一例外是
// Sub2API 权威标记为 automatic 的已关闭账号，调度器会对它做低频真实生成验证以尝试恢复。
const (
	StrategyModeHealthProbe    = "health_probe"
	StrategyModeMultiplierOnly = "multiplier_only"
)

// ProbeMode 控制探活请求的类型。real_model 发送真实模型请求（max_tokens=1），消耗 token；
// models_endpoint 只请求 GET /v1/models 验证 key 有效性和端点可达性，不消耗 token。
const (
	ProbeModeRealModel      = "real_model"
	ProbeModeModelsEndpoint = "models_endpoint"
)
...
// ManualUpstreamKeyMultiplier is a fallback display value for a target whose
// upstream API-key group multiplier cannot be discovered automatically.
type ManualUpstreamKeyMultiplier struct {
	UserID         string  `json:"-"`
...
type ManualUpstreamKeyMultiplierInput struct {
	Multiplier *float64 `json:"multiplier"`
}

// UpstreamKeyMultiplierHistoryPoint is a server-side aggregation of immutable
// hourly observations. Day and week views are averages of those hourly samples.
type UpstreamKeyMultiplierHistoryPoint struct {
...
type upstreamKeyMultiplierHistoryRecord struct {
	UserID         string
...
// Sub2APIRecoveryActioner asks Sub2API to atomically recover an account that it
// itself stopped scheduling (schedulability_source=automatic), after a completed
// real-generation probe has succeeded.
//
// expectedChangedAt carries the schedulability_changed_at value observed when the
// probe was enqueued. Sub2API uses it as a compare-and-set version, so a late
// recovery request cannot overwrite an administrator who took manual ownership
// while the probe was still running.
type Sub2APIRecoveryActioner interface {
	RecoverSub2APIAdminAccountSchedulability(
...
// PlatformGroupStatusWriter is separate from PlatformGroupReader because NewAPI
// can list group labels but does not expose a group-level enable switch.
type PlatformGroupStatusWriter interface {
	UpdateAdminGroupStatus(session upstream.Session, groupID string, active bool) error
}
