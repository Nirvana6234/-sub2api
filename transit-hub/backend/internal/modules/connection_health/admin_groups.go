...
type AdminGroupHealth struct {
	...
	StatusMutable         bool                    `json:"statusMutable"`
	...
	PriorityStrategy      string                  `json:"priorityStrategy"`
...
// SetAdminGroupStatus changes the real upstream group state after verifying
// that the group belongs to the current admin workspace.
func (s *Service) SetAdminGroupStatus(ctx context.Context, userID string, groupID string, active bool) error {
	if s.platformGroups == nil || s.groupStatus == nil {
		return errors.New("connection_health: upstream group status is not supported")
	}
	...
	for _, group := range groups {
		if group.ID == groupID {
			return s.groupStatus.UpdateAdminGroupStatus(session, groupID, active)
		}
	}
...
type AdminGroupAccount struct {
	...
	SchedulerScore          *float64   `json:"schedulerScore,omitempty"`
	UsageP95FirstTokenMs    *int       `json:"usageP95FirstTokenMs,omitempty"`
	UsageSampleCount        int        `json:"usageSampleCount,omitempty"`
	...
	UpstreamKeyGroupMultiplierSource string   `json:"upstreamKeyGroupMultiplierSource,omitempty"`
	// 独立探活字段。
...
func (s *Service) AdminGroups(ctx context.Context, userID string) ([]AdminGroupHealth, error) {
	...
	manualMultipliers, err := s.repo.ListManualUpstreamKeyMultipliers(ctx, userID, adminAccountID)
	if err != nil {
		return nil, err
	}
	...
	manualMultiplierByTarget := make(map[string]float64, len(manualMultipliers))
	for _, value := range manualMultipliers {
		manualMultiplierByTarget[value.TargetID] = value.Multiplier
	}
	...
	for _, group := range groups {
		health := AdminGroupHealth{
			...
			Status:            group.Status,
			StatusMutable:     session.Platform == upstream.PlatformSub2API,
		...
		for _, policyID := range health.AssignedPolicyIDs {
			if policy, ok := policyByID[policyID]; ok && policy.Enabled {
				switch normalizePriorityMode(policy.PriorityMode) {
				case PriorityModeAuto:
					health.PriorityMode = PriorityModeAuto
				case PriorityModeMultiplier:
					if health.PriorityMode != PriorityModeAuto {
		...
		if health.PriorityMode == PriorityModeAuto {
			assignedPolicies := make([]Policy, 0, len(health.AssignedPolicyIDs))
			for _, policyID := range health.AssignedPolicyIDs {
				if policy, ok := policyByID[policyID]; ok {
					assignedPolicies = append(assignedPolicies, policy)
				}
			}
			health.PriorityStrategy = priorityStrategyForPolicies(assignedPolicies)
		}
		...
		for _, acc := range accounts {
			...
			upstreamKeyMultiplier := upstreamKeyGroup.multiplier
			upstreamKeyMultiplierSource := ""
			if upstreamKeyMultiplier != nil {
				upstreamKeyMultiplierSource = upstreamKeyGroup.source
				if upstreamKeyMultiplierSource == "" {
					upstreamKeyMultiplierSource = "detected"
				}
			} else if manualValue, ok := manualMultiplierByTarget[targetID]; ok {
				value := manualValue
				upstreamKeyMultiplier = &value
				upstreamKeyMultiplierSource = "manual"
			}
			...
			priorityStateKey := prioritySyncTargetID(platform, group.ID, targetID)
			priorityState, priorityManaged := priorityByTarget[priorityStateKey]
			...
			displayPriority := acc.Priority
			if session.Platform == upstream.PlatformSub2API && acc.GroupPriority != nil {
				displayPriority = acc.GroupPriority
			}
			item := AdminGroupAccount{
				...
				SchedulerScore:                   acc.SchedulerScore,
				UsageP95FirstTokenMs:             acc.UsageP95FirstTokenMs,
				UsageSampleCount:                 acc.UsageSampleCount,
				...
				UpstreamKeyGroupMultiplier:       upstreamKeyMultiplier,
				UpstreamKeyGroupMultiplierSource: upstreamKeyMultiplierSource,
				TargetID:                         targetID,
...
type latestMultiplierHistoryReader interface {
	ListUpstreamKeyMultiplierHistory(context.Context, string, string, string, string) ([]UpstreamKeyMultiplierHistoryPoint, error)
}

// upstreamKeyGroupsByAdminAccount uses the same persisted source as the
// upstream-key multiplier history chart. It deliberately does not request the
// upstream key or group APIs while rendering the health page.
func (s *Service) upstreamKeyGroupsByAdminAccount(
	...
	if err != nil {
		log.Printf("[connection-health] upstream key multiplier history lookup skipped workspace=%s err=%v", adminAccountID, err)
		return result
	}
	conflictedAccounts := make(map[string]bool)
	// 倍率快照按站点缓存：同一站点的所有渠道共用一次上游查询，与采集器的做法
	// 一致，避免一个站点下每多一个渠道就多打一次上游接口。
	liveReader, hasLiveReader := s.mySites.(liveUpstreamKeyGroupReader)
	liveBySite := make(map[string]map[string]my_sites.UpstreamKeyGroupSnapshot)
	liveFailedSites := make(map[string]bool)
	for _, connection := range connections {
		...
		if accountID == "" || conflictedAccounts[accountID] {
			continue
		}
		info, ok := s.latestHistoryUpstreamKeyGroupForConnection(ctx, connection, adminPlatform)
		if !ok && hasLiveReader {
			// 历史为空不代表倍率不可知：倍率快照采集器每小时才跑一次，刚配好的
			// 渠道要等到下一个整点才会有历史行，而它的倍率此刻已经完全可解析
			// （渠道 → key → 分组 → 分组倍率）。没有这个回退，新渠道会在页面上
			// 显示"关联后展示倍率"最长一小时，运营会以为关联没配好而反复检查。
			// 同样覆盖历史被清理、或某次采集失败的情况。
			info, ok = s.liveUpstreamKeyGroupForConnection(ctx, liveReader, connection, liveBySite, liveFailedSites)
		}
		if !ok {
			continue
		}
		if existing, exists := result[accountID]; exists && !sameUpstreamKeyGroup(existing, info) {
			delete(result, accountID)
			conflictedAccounts[accountID] = true
			continue
		}
...
// latestHistoryUpstreamKeyGroupForConnection reads the newest persisted
// multiplier sample for the health page. The history source is already
// classified as detected/manual by the collector.
func (s *Service) latestHistoryUpstreamKeyGroupForConnection(
	ctx context.Context,
	connection my_sites.RealConnection,
	adminPlatform string,
) (upstreamKeyGroupInfo, bool) {
	repository, ok := s.repo.(latestMultiplierHistoryReader)
	if !ok {
		return upstreamKeyGroupInfo{}, false
	}
	userID := strings.TrimSpace(connection.UserID)
	workspaceID := strings.TrimSpace(connection.WorkspaceAdminAccountID)
	accountID := strings.TrimSpace(connection.AdminAccountID)
	if userID == "" || workspaceID == "" || accountID == "" {
		return upstreamKeyGroupInfo{}, false
	}
	targetID := buildTargetID(adminPlatform, workspaceID, accountID)
	points, err := repository.ListUpstreamKeyMultiplierHistory(ctx, userID, workspaceID, targetID, "hour")
	...
	value := latest.Multiplier
	source := strings.TrimSpace(latest.Source)
	if source == "" {
		source = "detected"
	}
	return upstreamKeyGroupInfo{
		...
		name:       strings.TrimSpace(connection.UpstreamGroupName),
		multiplier: &value,
		source:     source,
	}, true
}
...
// liveUpstreamKeyGroupForConnection 直接从上游站点解析该渠道当前的分组倍率，
// 供倍率历史尚未落下第一行时使用。
//
// 它与倍率快照采集器读同一个数据源（站点的 key 列表 × 分组倍率表），因此结果
// 与下一个整点将要写入历史的值一致——这里只是把"下一小时才可见"提前到"配好
// 即可见"，不引入第二套口径。source 同样标记为 detected：值确实来自对上游的
// 观测，只是还没有被持久化成历史样本。
//
// 每个站点最多查询一次并缓存在调用方传入的 map 里；查询失败的站点记入
// failedSites，同一次请求内不再重试。
func (s *Service) liveUpstreamKeyGroupForConnection(
	ctx context.Context,
	reader liveUpstreamKeyGroupReader,
	connection my_sites.RealConnection,
	snapshotsBySite map[string]map[string]my_sites.UpstreamKeyGroupSnapshot,
	...
	siteID := strings.TrimSpace(connection.UpstreamSiteID)
	keyID := strings.TrimSpace(connection.UpstreamKeyID)
	if userID == "" || workspaceID == "" || siteID == "" || keyID == "" {
		return upstreamKeyGroupInfo{}, false
	}
	...
	if !loaded {
		if failedSites[cacheKey] {
			return upstreamKeyGroupInfo{}, false
		}
		items, err := reader.ListUpstreamKeyGroupSnapshotsForWorkspace(ctx, userID, workspaceID, siteID)
		if err != nil {
			// 采集器在这条路径上是完全静默的，运营只能看到"关联后展示倍率"却
			// 无从判断是还没采集还是上游读不到。这里留一行可检索的日志。
			log.Printf("[connection-health] live upstream key multiplier lookup failed site_id=%s err=%v", siteID, err)
	...
	snapshot, found := values[keyID]
	if !found || snapshot.Multiplier == nil {
		return upstreamKeyGroupInfo{}, false
	}
	value := *snapshot.Multiplier
	name := strings.TrimSpace(snapshot.GroupName)
	if name == "" {
		// 上游的 key 列表可能只带 group_id 不带 group_name，此时沿用渠道配置时
		// 记下的分组名，避免页面显示成空白。
		name = strings.TrimSpace(connection.UpstreamGroupName)
	}
