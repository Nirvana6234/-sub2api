...
func TestComputeReferenceMultipliers(t *testing.T) {
	...
	t.Run("average: skips missing aggregate targets when at least one multiplier is available", func(t *testing.T) {
		targets := []UpstreamGroupRef{
			{SiteID: "sync1", GroupName: "A"},
			{SiteID: "sync1", GroupName: "missing"},
			{SiteID: "other", GroupName: "missing-other"},
		}
		changesByGroup := map[string]groupMultiplierChange{
			"A": {Old: 1.00, New: 1.20},
		}
		...
		if newRef != 1.20 {
			t.Errorf("newRef = %.4f, want 1.2000", newRef)
		}
	})

}

func TestComputeCurrentReferenceMultiplierSkipsMissingAggregateTargets(t *testing.T) {
	mapping := GroupMapping{
		AutoPricingSource: "average_upstream",
		UpstreamTargets: []UpstreamGroupRef{
			{SiteID: "site-a", GroupName: "a"},
			{SiteID: "site-b", GroupName: "missing"},
		},
	}

	ref, ok, reason := computeCurrentReferenceMultiplier(mapping, staticLookup(map[string]float64{
		"site-a|a": 2.0,
	}))
	if !ok {
		t.Fatalf("expected ok=true, got reason=%s", reason)
	...
	if ref != 2.0 {
		t.Fatalf("ref = %.4f, want 2.0000", ref)
	}

	_, ok, reason = computeCurrentReferenceMultiplier(mapping, staticLookup(nil))
	if ok {
		t.Fatal("expected ok=false when every aggregate target is missing")
	}
	if reason != "missing_reference_multiplier" {
		t.Fatalf("reason = %s, want missing_reference_multiplier", reason)
	}
}
