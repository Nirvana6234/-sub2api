...
func TestRunAutoPricingNowCalculatesCurrentReference(t *testing.T) {
	...
	server, _ := newNewAPITestServer(t, map[string]float64{"vip": 1.1, "a": 1.0, "b": 3.0}, []string{"vip", "a", "b"})
	defer server.Close()
	repo.state.Session = testSession(server.URL)
	upstreamSession := testSession(server.URL)
	lookup := testUpstreamLookup{sites: map[string]*upstream.Site{
		"site-a": {ID: "site-a", UserID: "user-1", AdminAccountID: "admin-1", Status: upstream.StatusConnected, Session: &upstreamSession, Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{Name: "a", Multiplier: floatPtr(1.0)}}}},
		"site-b": {ID: "site-b", UserID: "user-1", AdminAccountID: "admin-1", Status: upstream.StatusConnected, Session: &upstreamSession, Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{Name: "b", Multiplier: floatPtr(3.0)}}}},
	}}
	service := NewService(repo, upstream.NewPlatformService(upstream.NewHTTPClient(server.Client())), lookup)
	service.SetAdminAccountResolver(testAdminResolver{currentID: "admin-1"})
...
func TestMappingOptionsDoesNotTreatCachedMissingTargetsAsAuthoritative(t *testing.T) {
	...
	if len(response.StaleTargets) != 0 {
		t.Fatalf("expected cached target absence not to be treated as authoritative, got %#v", response.StaleTargets)
	}
...
func TestMappingOptionsAndPricingLookupReadLiveUpstreamMultiplier(t *testing.T) {
	server, _ := newNewAPITestServer(t, map[string]float64{"vip": 2.5}, []string{"vip"})
	defer server.Close()
	...
	repo := &testStateRepo{state: &State{
		...
		Mappings: []GroupMapping{{
			OwnGroup:        "vip",
			UpstreamTargets: []UpstreamGroupRef{{SiteID: "site-a", GroupName: "vip"}},
		}},
	}}
	lookup := testUpstreamLookup{sites: map[string]*upstream.Site{
		"site-a": {
			ID: "site-a", UserID: "user-1", AdminAccountID: "admin-1", Status: upstream.StatusConnected,
			Session: &upstreamSession,
			Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{Name: "vip", Multiplier: floatPtr(1.0)}}},
		},
	}}
	service := NewService(repo, upstream.NewPlatformService(upstream.NewHTTPClient(server.Client())), lookup)
	service.SetAdminAccountResolver(testAdminResolver{currentID: "admin-1"})
	...
	if len(response.UpstreamTargetMultipliers) != 1 {
		t.Fatalf("expected one live target multiplier, got %#v", response.UpstreamTargetMultipliers)
	}
	live := response.UpstreamTargetMultipliers[0]
	if live.Stale || live.Multiplier == nil || *live.Multiplier != 2.5 {
		t.Fatalf("expected live 2.5 multiplier rather than cached 1.0, got %#v", live)
	}

	lookupMultiplier := service.buildWorkspaceLookupMultiplier(context.Background(), "user-1", "admin-1")
	value := lookupMultiplier("site-a", "vip")
	if value == nil || *value != 2.5 {
		t.Fatalf("expected pricing lookup to use live 2.5 multiplier, got %v", value)
	}
}

func TestMappingOptionsKeepsCachedMultiplierWhenLiveReadFails(t *testing.T) {
	adminServer, _ := newNewAPITestServer(t, map[string]float64{"vip": 1.1}, []string{"vip"})
	...
	const cachedMultiplier = 2.5
	repo := &testStateRepo{state: &State{
		UserID:         "user-1",
		AdminAccountID: "admin-1",
		Session:        testSession(adminServer.URL),
		Mappings: []GroupMapping{{
			OwnGroup:        "vip",
			UpstreamTargets: []UpstreamGroupRef{{SiteID: "site-a", GroupName: "vip"}},
		}},
	...
	lookup := testUpstreamLookup{sites: map[string]*upstream.Site{
		"site-a": {
			...
			Session: &upstream.Session{Platform: upstream.PlatformNewAPI, BaseURL: upstreamServer.URL},
			Metrics: upstream.Metrics{Groups: []upstream.GroupInfo{{Name: "vip", Multiplier: floatPtr(cachedMultiplier)}}},
		},
	}}
	service := NewService(repo, upstream.NewPlatformService(upstream.NewHTTPClient(adminServer.Client())), lookup)
	service.SetAdminAccountResolver(testAdminResolver{currentID: "admin-1"})
	...
	if len(response.UpstreamTargetMultipliers) != 1 {
		t.Fatalf("expected one target multiplier, got %#v", response.UpstreamTargetMultipliers)
	}
	target := response.UpstreamTargetMultipliers[0]
	if target.Stale || target.Source != "cached" || target.Multiplier == nil || *target.Multiplier != cachedMultiplier {
		t.Fatalf("expected cached multiplier after live read failure, got %#v", target)
	}
}
...
func TestMappingOptionsBackfillsRealConnectionWithoutPersisting(t *testing.T) {
	...
	connRepo := &testConnRepo{
		stateRepo:  repo,
		connection: &RealConnection{ID: "conn-1", UserID: "user-1", WorkspaceAdminAccountID: "admin-1", UpstreamSiteID: "site-a", UpstreamGroupName: "g1", OwnGroupIDs: []string{"vip"}, ProvisioningMode: ProvisioningModeLegacy},
	}
	server, _ := newNewAPITestServer(t, map[string]float64{"vip": 1.1}, []string{"vip"})
	defer server.Close()
	repo.state.Session = testSession(server.URL)
...
func TestMappingOptionsDoesNotBackfillManagedConnectionAfterMappingRemoval(t *testing.T) {
	repo := &testStateRepo{state: &State{
		UserID:         "user-1",
		AdminAccountID: "admin-1",
		Session:        testSession(""),
	...
	connRepo := &testConnRepo{
		stateRepo:  repo,
		connection: &RealConnection{ID: "conn-1", UserID: "user-1", WorkspaceAdminAccountID: "admin-1", UpstreamSiteID: "site-a", UpstreamGroupName: "g1", OwnGroupIDs: []string{"vip"}, ProvisioningMode: ProvisioningModeManaged, PricingMappingEnabled: true},
	}
	server, _ := newNewAPITestServer(t, map[string]float64{"vip": 1.1}, []string{"vip"})
	defer server.Close()
	repo.state.Session = testSession(server.URL)
	...
	if len(response.Mappings) != 0 {
		t.Fatalf("managed connection must not restore a removed mapping, got %#v", response.Mappings)
	}
}
