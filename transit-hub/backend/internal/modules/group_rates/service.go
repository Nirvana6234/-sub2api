...
type GroupRateSnapshotRefresherForSite interface {
	RefreshGroupRateSnapshot(ctx context.Context, userID, adminAccountID, siteID string) error
}
...
func (s *Service) History(ctx context.Context, userID string, adminAccountID string, siteID string, groupName string, platform string) ([]HistoryRow, error) {
	if refresher, ok := s.refresher.(GroupRateSnapshotRefresherForSite); ok {
		if err := refresher.RefreshGroupRateSnapshot(ctx, strings.TrimSpace(userID), strings.TrimSpace(adminAccountID), strings.TrimSpace(siteID)); err != nil {
			log.Printf("refresh group rate snapshot site=%s: %v", siteID, err)
		}
	}
