...
func TestResolveProbeCredential_Sub2APIOpenAIOAuthUsesCodexResponsesFallback(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, map[string]any{"data": []map[string]any{
			{"name": "codex", "platform": "openai", "type": "oauth", "credentials": map[string]any{"access_token": "oauth-secret"}},
		}})
	}))
	defer server.Close()
	...
	service := NewPlatformService(NewHTTPClient(server.Client()))
	session := Session{Platform: PlatformSub2API, BaseURL: server.URL, AccessToken: "tok"}
	account := AdminGroupAccountInfo{ID: "55", Name: "codex", Platform: "openai", Type: "oauth", Models: "gpt-5.6-sol"}

	cred, err := service.ResolveProbeCredential(session, account)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if cred.BaseURL != chatGPTCodexResponsesURL || cred.Key != "oauth-secret" || cred.AccountPlatform != "openai" || cred.AccountType != "oauth" {
		t.Fatalf("unexpected credential: %+v", cred)
	}
}

func TestResolveProbeCredential_Sub2APIPreservesOpenAIExtraAndHeaderOverrides(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, map[string]any{"data": []map[string]any{
			{
				"name": "api", "platform": "openai", "type": "apikey",
	...
	service := NewPlatformService(NewHTTPClient(server.Client()))
	session := Session{Platform: PlatformSub2API, BaseURL: server.URL, AccessToken: "tok"}
	account := AdminGroupAccountInfo{ID: "55", Name: "api", Platform: "openai", Type: "apikey"}
