...
// ManualAccountingRepository is intentionally optional so existing in-memory
// repositories used by focused service tests do not need a database account.
type ManualAccountingRepository interface {
	AddRecharge(ctx context.Context, userID, adminAccountID, siteID string, entry RechargeEntry) error
	ListRecharges(ctx context.Context, userID, adminAccountID, siteID string) ([]RechargeEntry, error)
	UpsertDailyUsage(ctx context.Context, siteID, usageDate, groupName string, rawAmount, multiplier, adjustedAmount float64) error
	ManualAccountingSummary(ctx context.Context, userID, adminAccountID, siteID string) (ManualAccountingSummary, error)
}
...
func NewService(platformService *PlatformService, repository SiteRepository, snapshotWriter SnapshotWriter, cache SiteCache) *Service {
	return &Service{
		...
		groupRateCache:  make(map[string]groupRateCacheEntry),
		groupRateFetch:  make(map[string]*groupRateFetch),
	}
}
...
// AddRecharge enables the manual accounting mode and appends an immutable
// balance entry. The first entry is normally the historical balance entered by
// the operator; later entries are ordinary recharges.
func (s *Service) AddRecharge(ctx context.Context, userID, siteID string, dto CreateRechargeRequest) (Response, error) {
	if dto.Amount <= 0 {
		return Response{}, invalidBodyError("amount")
	}
	accounting, ok := s.repository.(ManualAccountingRepository)
	if !ok {
		return Response{}, errors.New("manual accounting repository unavailable")
	}
	site, adminAccountID, err := s.ownedSite(ctx, userID, siteID)
	if err != nil {
		return Response{}, err
	}
	id, err := randomID()
	...
	entry := RechargeEntry{ID: id, Amount: dto.Amount, Note: strings.TrimSpace(dto.Note), CreatedAt: time.Now().UnixMilli()}
	if err := accounting.AddRecharge(ctx, userID, adminAccountID, siteID, entry); err != nil {
		return Response{}, err
	}
	site.Settings.ManualAccountingEnabled = true
	if err := s.applyManualAccountingSummary(ctx, accounting, site); err != nil {
		return Response{}, err
	}
...
func (s *Service) ListRecharges(ctx context.Context, userID, siteID string) ([]RechargeEntry, error) {
	accounting, ok := s.repository.(ManualAccountingRepository)
	if !ok {
		return nil, errors.New("manual accounting repository unavailable")
	}
	_, adminAccountID, err := s.ownedSite(ctx, userID, siteID)
	if err != nil {
		return nil, err
	}
	return accounting.ListRecharges(ctx, userID, adminAccountID, siteID)
}
...
func (s *Service) KeyUsageToday(ctx context.Context, userID string) ([]KeyUsageTodayItem, error) {
	...
	return s.KeyUsageTodayForAccount(ctx, userID, adminAccountID)
}

// KeyUsageTodayForAccount returns today's per-key usage for an explicit workspace.
// Dashboard cards, drill-down details, and daily snapshots must share this source.
func (s *Service) KeyUsageTodayForAccount(ctx context.Context, userID, adminAccountID string) ([]KeyUsageTodayItem, error) {
	return s.KeyUsageForAccountDate(ctx, userID, adminAccountID, reportingDate(time.Now()))
}

// KeyUsageForAccountDate collects a specified calendar day for one workspace.
// Historical dashboard reconciliation never substitutes current-day usage.
func (s *Service) KeyUsageForAccountDate(ctx context.Context, userID, adminAccountID, date string) ([]KeyUsageTodayItem, error) {
	if strings.TrimSpace(adminAccountID) == "" {
		return nil, newRequestError("admin.adminAccounts.errors.noCurrentAccount", "")
	}
	sites, err := s.cache.ListByUser(ctx, userID)
	if err != nil {
		return nil, err
	...
	for _, site := range targets {
		...
		go func(site *Site) {
			...
			stats, fetchErr := s.platformService.FetchKeyUsageForDate(refreshedSession, groups, date)
			if fetchErr != nil {
...
func (s *Service) Create(ctx context.Context, userID string, dto CreateRequest) (Response, error) {
	...
	platform := resolvedPlatformForAuthMode(dto.Platform, dto.AuthMode)
...
func (s *Service) Update(ctx context.Context, userID string, id string, dto UpdateRequest) (Response, error) {
	...
	site.Platform = resolvedPlatformForAuthMode(dto.Platform, dto.AuthMode)
...
func resolvedPlatformForAuthMode(platform Platform, authMode AuthMode) Platform {
	if normalizedAuthMode(authMode) == AuthModeToken && platform != PlatformNewAPI {
		return PlatformSub2API
	}
	return resolvedPlatform(platform)
}
...
func (s *Service) sync(ctx context.Context, id string) (Response, error) {
	...
	if refreshErr == nil {
		...
		if refreshErr == nil && site.Settings.ManualAccountingEnabled {
			refreshErr = s.refreshManualAccounting(ctx, site, refreshedSession, &metrics)
		}
		if refreshErr != nil && site.Settings.ManualAccountingEnabled && site.Platform == PlatformSub2API {
			// A restricted account can fail metadata sync while /keys and per-key
			// usage remain readable. Keep the sync error but update its ledger.
			if err := s.refreshManualAccountingFromKeyUsage(ctx, site, refreshedSession); err != nil {
				log.Printf("manual accounting fallback failed site_id=%s err=%v", site.ID, err)
			} else {
				// The normal metadata refresh is still an error, but the manual
				// ledger values must survive the later cache re-read.
				_ = s.setCachedSite(ctx, site)
			}
		}
	}
	...
	if refreshErr != nil {
		...
		site.ErrorKey = &key
		if site.Settings.ManualAccountingEnabled && site.Platform == PlatformSub2API {
			// Preserve a refreshed credential even when only metadata sync failed.
			site.Session = &refreshedSession
		}
	} else {
		now := time.Now().UnixMilli()
		...
		s.rememberCurrentGroups(site.UserID+"|"+site.AdminAccountID+"|"+site.ID, metrics.Groups, time.Now())
		site.Status = StatusConnected
		site.ErrorKey = nil
		site.LastSyncedAt = &now
	}
...
func (s *Service) UpdateSettings(ctx context.Context, userID string, siteID string, dto SiteSettings) (Response, error) {
	...
	// Keep the existing accounting mode intact because older clients do not send
	// that field, but accept TransitHub-owned manual group multipliers.
	site.Settings.BalanceThreshold = dto.BalanceThreshold
	if dto.ManualGroupMultipliers != nil {
		multipliers, err := normalizeManualGroupMultipliers(dto.ManualGroupMultipliers)
		if err != nil {
			return Response{}, err
		}
		site.Settings.ManualGroupMultipliers = multipliers
	}
	_ = s.setCachedSite(ctx, site)
	if saveErr := s.saveSite(ctx, site); saveErr != nil {
		return Response{}, saveErr
...
func (s *Service) ownedSite(ctx context.Context, userID, siteID string) (*Site, string, error) {
	site, err := s.cache.Get(ctx, siteID)
	if err != nil {
		return nil, "", err
	}
	...
	return site, adminAccountID, nil
}

func (s *Service) refreshManualAccounting(ctx context.Context, site *Site, session Session, metrics *Metrics) error {
	if metrics == nil {
		return nil
	}
	accounting, ok := s.repository.(ManualAccountingRepository)
	if !ok {
		return errors.New("manual accounting repository unavailable")
	}
	if site.Platform != PlatformSub2API {
		return s.applyManualAccountingSummary(ctx, accounting, site, metrics)
	}
	stats, err := s.platformService.FetchSub2APIGroupDailyStats(session, metrics.Groups)
	if err != nil {
		return err
	}
	multipliers := make(map[string]float64, len(metrics.Groups))
	for _, group := range metrics.Groups {
		if group.Multiplier != nil && *group.Multiplier >= 0 {
			multipliers[strings.TrimSpace(group.Name)] = *group.Multiplier
		}
	}
	todayTotal := 0.0
	// 日界与营收侧、reportingDate 共用 Asia/Shanghai。此前用 time.Now() 的服务器
	// 本地时区，容器时区一变，手工记账的"今天"就和其他口径错开一天。
	usageDate := reportingDate(time.Now())
	for _, stat := range stats {
		groupName := strings.TrimSpace(stat.GroupName)
		if groupName == "" || stat.TodayActualCost < 0 {
			continue
		}
		multiplier, found := multipliers[groupName]
		if !found {
			// A missing rate must remain a conservative one-to-one cost rather
			// than silently turning actual upstream consumption into zero.
			multiplier = 1
		}
		// 待确认的重复计价风险：TodayActualCost 若取自上游 sub2api 的
		// /api/v1/admin/groups/usage-summary，其 today_cost = SUM(usage_logs.actual_cost)，
		// 而 actual_cost 的定义是「标准成本 × 分组倍率」——倍率已经含在里面。
		// 此时再乘一次 metrics.Groups 上探测到的同一个分组倍率，会让倍率被平方
		// （5x 分组记成 25 倍成本）。
		//
		// 之所以暂不改动：TestManualAccountingUsesSub2APIGroupMultiplier 明确保护
		// 「运营把分组手工设成 0 倍率即不计成本」的能力，说明该字段至少在部分
		// 场景下被当作运营可控的定价参数而非纯探测值；两种语义需要先分开表达
		// （探测值 vs 手工定价）才能安全修复。当前 upstream_site_daily_usage 在
		// 两套生产环境都是 0 行，这条路径没有数据流经，不构成现网风险。
		adjusted := stat.TodayActualCost * multiplier
		if err := accounting.UpsertDailyUsage(ctx, site.ID, usageDate, groupName, stat.TodayActualCost, multiplier, adjusted); err != nil {
			return err
		}
		todayTotal += adjusted
	}
	metrics.TodayConsume = metric(&todayTotal)
	return s.applyManualAccountingSummary(ctx, accounting, site, metrics)
}

// refreshManualAccountingFromKeyUsage is deliberately narrower than normal
// Sub2API sync: it needs only /keys and each key's usage endpoint, and prices
// usage exclusively with TransitHub-entered multipliers. Missing configuration
// is never silently treated as 1x.
func (s *Service) refreshManualAccountingFromKeyUsage(ctx context.Context, site *Site, session Session) error {
	accounting, ok := s.repository.(ManualAccountingRepository)
	if !ok {
		return errors.New("manual accounting repository unavailable")
	}
	multipliers, err := normalizeManualGroupMultipliers(site.Settings.ManualGroupMultipliers)
	if err != nil {
		return err
	}
	if len(multipliers) == 0 {
		return errors.New("manual group multipliers required for fallback accounting")
	}

	usageDate := reportingDate(time.Now())
	keyStats, err := s.platformService.FetchKeyUsageForDate(session, nil, usageDate)
	if err != nil {
		return err
	}
	...
	for _, stat := range keyStats {
		groupName := strings.TrimSpace(stat.GroupName)
		if groupName == "" || stat.TodayAmount < 0 {
			continue
		}
		byGroup[groupName] += stat.TodayAmount
	}

	todayTotal := 0.0
	missingMultiplier := false
	for groupName, rawAmount := range byGroup {
		multiplier, found := multipliers[groupName]
		if !found {
			missingMultiplier = true
			log.Printf("manual accounting fallback skipped unpriced group site_id=%s group=%s", site.ID, groupName)
			continue
		}
		adjusted := rawAmount * multiplier
		if err := accounting.UpsertDailyUsage(ctx, site.ID, usageDate, groupName, rawAmount, multiplier, adjusted); err != nil {
			return err
		}
		todayTotal += adjusted
	}
	...
	if !missingMultiplier {
		site.Metrics.TodayConsume = metric(&todayTotal)
	}
	return nil
}

func normalizeManualGroupMultipliers(values map[string]float64) (map[string]float64, error) {
	if values == nil {
		return nil, nil
	}
	normalized := make(map[string]float64, len(values))
	for groupName, multiplier := range values {
		groupName = strings.TrimSpace(groupName)
		if groupName == "" || multiplier < 0 {
			return nil, invalidBodyError("manualGroupMultipliers")
		}
		normalized[groupName] = multiplier
	}
	return normalized, nil
}

func (s *Service) applyManualAccountingSummary(ctx context.Context, accounting ManualAccountingRepository, site *Site, target ...*Metrics) error {
	summary, err := accounting.ManualAccountingSummary(ctx, site.UserID, site.AdminAccountID, site.ID)
	if err != nil {
		return err
	}
	...
	balance := summary.RechargeTotal - summary.ConsumedTotal
	metrics.Balance = metric(&balance)
	metrics.HistoryRecharge = metric(&summary.RechargeTotal)
	return nil
}
