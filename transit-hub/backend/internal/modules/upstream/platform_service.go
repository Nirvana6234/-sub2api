...
func (s *PlatformService) FetchSub2APIUpstreamErrorEvents(session Session, since time.Time, now time.Time) ([]UpstreamErrorEvent, error) {
	...
	query := url.Values{}
	query.Set("start_time", since.UTC().Format(time.RFC3339))
	query.Set("end_time", now.UTC().Format(time.RFC3339))
	query.Set("page", "1")
	query.Set("page_size", "500")
	query.Set("status_codes", "502,503")
	query.Set("sort_by", "created_at")
	query.Set("sort_order", "desc")
	response, err := s.httpClient.requestJSON(session.BaseURL+"/api/v1/admin/ops/errors?"+query.Encode(), adminAuthOptions(session))
	if err != nil {
		return nil, err
	}
	items := dataArray(response.Payload)
	events := make([]UpstreamErrorEvent, 0, len(items))
	for _, item := range items {
		record := dataRecord(item)
		status := firstInt(record, []string{"status_code", "statusCode"})
		if status == nil || (*status != http.StatusBadGateway && *status != http.StatusServiceUnavailable) {
			continue
		}
		createdAt := parseFlexibleTime(firstAny(record, []string{"created_at", "createdAt"}))
		if createdAt == nil || createdAt.Before(since) || createdAt.After(now) {
		...
		events = append(events, UpstreamErrorEvent{
			GroupID:    strings.TrimSpace(firstStringy(record, []string{"group_id", "groupId"})),
			GroupName:  strings.TrimSpace(firstStringy(record, []string{"group_name", "groupName"})),
			StatusCode: *status,
			Message:    strings.TrimSpace(firstStringy(record, []string{"message", "error_message", "errorMessage"})),
			Model:      strings.TrimSpace(firstStringy(record, []string{"requested_model", "requestedModel", "model"})),
			CreatedAt:  *createdAt,
		})
	}
	return events, nil
