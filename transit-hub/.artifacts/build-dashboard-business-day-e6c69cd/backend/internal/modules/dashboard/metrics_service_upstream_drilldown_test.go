...
func (f *fakeUpstreamLister) KeyUsageTodayForAccount(ctx context.Context, userID, adminAccountID string) ([]upstream.KeyUsageTodayItem, error) {
	return f.keyUsageItems, f.keyUsageErr
}

func (f *fakeUpstreamLister) KeyUsageForAccountDate(ctx context.Context, userID, adminAccountID, date string) ([]upstream.KeyUsageTodayItem, error) {
	f.keyUsageDate = date
	return f.keyUsageItems, f.keyUsageErr
}
...
func TestTodayPurchaseForAccount_UsesKeyUsageSource(t *testing.T) {
	upstreams := &fakeUpstreamLister{
		keyUsageItems: []upstream.KeyUsageTodayItem{
			{SiteID: "site-a", KeyID: "key-a", TodayAmount: 0.67},
			{SiteID: "site-b", KeyID: "key-b", TodayAmount: 9.35},
		},
	}
	service := NewMetricsService(nil, nil, upstreams, nil, nil)

	got := service.todayPurchaseForAccount(context.Background(), "user-1", "account-1")
	if got != 10.02 {
		t.Fatalf("today purchase = %.2f, want 10.02 from key usage details", got)
	}
}

func TestPurchaseForAccountDateDoesNotSubstituteCurrentUsage(t *testing.T) {
	upstreams := &fakeUpstreamLister{keyUsageErr: errors.New("upstream unavailable")}
	service := NewMetricsService(nil, nil, upstreams, nil, nil)

	_, err := service.purchaseForAccountDate(context.Background(), "user-1", "account-1", "2026-08-04", false)
	if err == nil {
		t.Fatal("historical purchase must fail closed when same-day upstream usage is unavailable")
	}
	if upstreams.keyUsageDate != "2026-08-04" {
		t.Fatalf("requested date = %q, want historical date", upstreams.keyUsageDate)
	}
}
