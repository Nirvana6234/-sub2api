...
func (r *MetricsRepository) Upsert(ctx context.Context, snapshot DailySnapshot) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO dashboard_daily_stats (id, user_id, admin_account_id, date, today_profit, site_balance, today_purchase, net_profit, upstream_balance, created_at, is_finalized, finalized_at)
	...
	`, snapshot.ID, snapshot.UserID, snapshot.AdminAccountID, snapshot.Date, snapshot.TodayProfit, snapshot.SiteBalance,
		snapshot.TodayPurchase, snapshot.NetProfit, snapshot.UpstreamBalance, snapshot.CreatedAt, snapshot.IsFinalized, snapshot.FinalizedAt)
	return err
}
...
func (r *MetricsRepository) ListRange(ctx context.Context, userID, adminAccountID string, days int, today string) ([]DailySnapshot, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, user_id, admin_account_id, date, today_profit, site_balance, today_purchase, net_profit, upstream_balance, created_at, finalized_at, is_finalized
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.UserID, &s.AdminAccountID, &s.Date, &s.TodayProfit, &s.SiteBalance,
			&s.TodayPurchase, &s.NetProfit, &s.UpstreamBalance, &s.CreatedAt, &s.FinalizedAt, &s.IsFinalized); err != nil {
			return nil, err
		}
