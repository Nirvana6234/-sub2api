...
func TestFetchKeyUsageForDate_Sub2APIUsesExplicitShanghaiBusinessDay(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v1/keys":
			writeJSON(w, map[string]any{"data": []map[string]any{{"id": 1, "name": "key", "group": map[string]any{"name": "vip"}}}})
		case "/api/v1/usage/stats":
			query := r.URL.Query()
			if got := query.Get("start_date"); got != "2026-08-04" {
				t.Fatalf("start_date = %q, want 2026-08-04", got)
			}
			if got := query.Get("end_date"); got != "2026-08-04" {
				t.Fatalf("end_date = %q, want 2026-08-04", got)
			}
			if got := query.Get("timezone"); got != "Asia/Shanghai" {
	...
	service := NewPlatformService(NewHTTPClient(server.Client()))
	_, err := service.FetchKeyUsageForDate(Session{Platform: PlatformSub2API, BaseURL: server.URL, AccessToken: "token"}, nil, "2026-08-04")
	if err != nil {
		t.Fatalf("FetchKeyUsageForDate returned error: %v", err)
	}
}

func TestFetchKeyUsageForDate_NewAPIUsesShanghaiDayBoundaries(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/token/":
			writeJSON(w, map[string]any{"data": []map[string]any{{"id": 1, "name": "key", "group": "vip"}}, "total": 1})
		case "/api/log/self/stat":
			loc, err := time.LoadLocation("Asia/Shanghai")
			if err != nil {
				t.Fatal(err)
			}
			wantStart := strconv.FormatInt(time.Date(2026, time.August, 4, 0, 0, 0, 0, loc).Unix(), 10)
			wantEnd := strconv.FormatInt(time.Date(2026, time.August, 5, 0, 0, 0, 0, loc).Unix(), 10)
			query := r.URL.Query()
			if got := query.Get("start_timestamp"); got != wantStart {
				t.Fatalf("start_timestamp = %q, want %q", got, wantStart)
			}
			if got := query.Get("end_timestamp"); got != wantEnd {
				t.Fatalf("end_timestamp = %q, want %q", got, wantEnd)
			}
			writeJSON(w, map[string]any{"data": map[string]any{"quota": 100000}})
		default:
			t.Fatalf("unexpected path: %s", r.URL.Path)
		}
	}))
	...
	service := NewPlatformService(NewHTTPClient(server.Client()))
	session := Session{Platform: PlatformNewAPI, BaseURL: server.URL, Cookie: "session=abc", UserID: "1", QuotaPerUnit: 100000}
	_, err := service.FetchKeyUsageForDate(session, nil, "2026-08-04")
	if err != nil {
		t.Fatalf("FetchKeyUsageForDate returned error: %v", err)
	}
}
