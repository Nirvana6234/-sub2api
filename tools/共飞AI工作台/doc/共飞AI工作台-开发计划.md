...
#### A9 执行记录（2026-09-03）—— **凭据不落盘已实证；换分组待做**

##### 先纠正这一步原本写错的前提

A9 原文写的是「申请/复用托管 key」，但 **Chat PWA 是刻意不要 API key 的**（不存 key 就没有存 key 的问题）。所以我一度想让 codex 也走 JWT 直连。查下来走不通，两条实测：

- **codex 的 agent 循环＝它自己调模型。** 不存在「只要它的 agent 逻辑、模型调用走 Chat 现有通道」这种用法 —— 除非在中间写一层协议翻译，那正是我们一直躲开的活。
- **codex 0.144.2 只会说 Responses 一种线协议。** 从**真二进制**里查的：`chat/completions` 出现 **0 次**，只有 `/responses`。而 Paw 面只有 `/chat/completions`（`paw.go` 里明写 `"Paw accepts only the authenticated account session"`），后端所有 `/responses` 都挂 `apiKeyAuth`，连 Playground 都没有 `/responses`。

所以工作台这一侧必须有一把真 key。定下来的约束是：**登录后按需取、只在内存里传、绝不落盘、用完撤销**。

##### 「不落盘」光靠调用方守不住

实测：`account/login/start` 一送进去，**codex 自己就把明文 key 写进 `CODEX_HOME/auth.json`**（96 字节）。不管调用方多小心，文件就在那儿。
...
auth.json 存在: True
删除后还存在: False
turn 收束: True          ← 删掉之后照样跑完
assistant 正文: 'PONG'
结束时 auth.json 又被写回来了吗: False
```

**登录后立刻删掉，会话照常跑完一轮，codex 也不会把它写回来。** 于是 key 在磁盘上的存活时间被压到毫秒级。

落地为 `CodexHome::purge_credentials()`：删之前先用等长的零覆盖（删除只摘目录项，内容还在扇区上），由桥在**握手之后立刻**调用。**抹不掉就让这次会话起不来**（`HostError::CredentialLeak`）—— 承诺没兑现而用户不知道，比起不来糟得多。

验收测试 `the_key_does_not_survive_on_disk_and_the_session_still_works` 同时钉两件事：**文件确实没了**，且**会话照样能跑完一轮**。只验前者会得到一个「安全但不能用」的实现。

##### 尚未验证（真出问题第一个查这里）

只验过**一条连接上的一轮**。codex 在 **401 重试、断线重连、`thread/resume`** 时会不会回头重读 `auth.json`，**没有验过**。如果出现「删了之后长会话中途失效」，第一个怀疑对象就是它。

##### 客户端侧：一个纯新增文件

`src/client/paw/agentKey.ts`：`leaseWorkbenchKey()` / `revokeWorkbenchKey()`，走 `POST|DELETE /api/v1/keys`（JWT 组下，和小白端同一条路；创建的响应 DTO 里 `key` 是明文）。

- 复用现成的 `pawRequest`（带 JWT + 401 自动刷新重放）——**那套逻辑不要复制第二份**，会走散。它本来就已经导出了。
- 每次会话**新建一把**而不是复用长期 key：一次泄露的爆炸半径就是一次会话。
- 租期 **1 天**，是撤销失败（比如进程被强杀来不及发 DELETE）时的兜底。
- key 的名字是 `共飞工作台（自动创建，可随时删除）` —— 用户会在自己的 key 列表里看见它，**必须自解释**，一个看不懂来历的 key 只会让人以为账号被盗。
- 文件头写了三条铁律：不进任何存储、不打日志（连长度前缀都不打）、不进 React state。

`api.ts` **一行未改**，和 B 线零冲突。

##### 还没做

- **会话结束时撤销**：`revokeWorkbenchKey` 写好了，但桥的 `stop()` 里还没接 —— 那要把租约 id 一路带下去，等 A6 把界面接起来时一并做。
- **换分组**：`leaseWorkbenchKey(groupId)` 的参数留好了（注意**不传** `group_id` 与**传 null** 语义不同：前者「你决定」，后者「清掉绑定」），但「换分组后新会话用新分组、旧租约作废」这条验收**没验**。
- **`~/.codex` 全程不读不写**这半在 A1 就由私有 `CODEX_HOME` + `CodexHome` 类型保证了。
