...
- 跑完通知，放 v2；
- 手机上回应审批、打断正在跑的一轮、逐字流式输出，原因见 §3.3，后续怎么补见 §11；
...
- 本机通常有两个 `codex-browser-use-*` 管道，其中**只有能回应 `tools/list` 的那个**是 app-tools，另一个回 `No handler registered`。
...
5. **老会话的模型问题。** 9 月 13 日用 `gpt-6-astra` 建的老会话，在「本地代理（ChatGPT 订阅直连）」下每轮都报 `'gpt-6-astra' model is not supported when using Codex with a ChatGPT account`。会话设置显示 `gpt-5.5`、或者调用时显式传 `model:"gpt-5.5"`，都不起作用；新建会话就正常。这不是管道的问题，但手机端要把这类错误原文转给用户。
...
| 自动 | 改项目内代码、跑测试都能自动完成；需要联网（如 `npm install`、`git push`）或写项目外文件时，**这一轮停在电脑上等人确认**。v1 手机上只能提示「请到电脑上确认」（V-6） | 能读全盘文件、改项目目录；联网和越界操作要电脑上点确认 |
...
- **发现管道**：枚举 `\\.\pipe\` 下 `codex-browser-use-*`，逐个发送 `tools/list`，谁能正常回应就用谁。桌面版重启后管道名会变，所以**每次连接失败都要重新发现**，不能缓存名字。
...
- **接口**：v1 只包装三个工具：`ListThreads` / `SendMessage` / `NavigateTo`。内容走 rollout（§6），用不到 `read_thread`；跑完通知放 v2，也用不到 `wait_threads`。**不提供通用的 `CallTool(name, args)`**，白名单写死在类型里，想绕也绕不过去。版本闸门也只比对这三个工具的签名，其余工具怎么变都不影响 v1。
- **调用方会话**：见 §10 的 D-3。
...
2a. 发送类命令的签名验不过 → 拒绝（前提是 D-8 采用签名方案）。
...
| 指标 | 数值 | 对设计的影响 |
|---|---|---|
| 文件大小 | 中位数 335 KB，最大 **18.6 MB** | 不能每次全量发，要建轮次索引、分页 |
| 单行最大 | **2.3 MB**（一条命令的输出） | 命令输出必须截断，全文按需另取 |
| `item_started` | **0 条** | rollout 只记**已完成**的步骤：手机看不到"正在执行哪条命令"，也没有逐字输出 |
| 同一内容的两份记录 | `event_msg/item_completed`（给界面的）和 `response_item`（给模型的，含加密推理） | **只读 `item_completed`**，`response_item` 一律跳过 |
...
- **轮次索引**：助手第一次打开某条会话时，把整个文件扫一遍，建立「turnId → 起止偏移」的索引并缓存在内存里，之后只做增量更新。`open` 和 `history` 都靠索引直接跳到对应位置，不必每次重扫。18.6 MB 的文件扫一遍要多久，列入 V-9。
- **断线续传不需要任何一方缓存事件**：rollout 只追加，手机重连时带上最后收到的 `seq`，助手从那个字节偏移接着读就行。服务端因此完全无状态。
- **什么时候需要重新同步**：rollout 路径变了，或者文件长度比游标还小，就回 `resync`，手机丢掉这条会话的本地缓存，重新 `open`。
- **跟读方式**：`FileSystemWatcher` 负责即时触发，另有 1 秒轮询兜底，因为 Watcher 会漏事件。只跟读有手机订阅的会话，最多 5 条；没有订阅者 60 秒后停止跟读。
- **运行状态**：`task_started` 之后显示「运行中 · 已 42 秒」，`task_complete` / `turn_aborted` 之后显示空闲或出错。rollout 里没有 `systemError` 这类状态，所以运行中每 10 秒再用 `list_threads` 校准一次。
...
- **看不到正在执行的那一步**，因为 rollout 只记已完成的步骤。一条跑了 3 分钟的命令，手机只能看到「运行中 · 已 3 分钟」，等它跑完才出现。`read_thread` 能不能补上这一块，列入 V-10。
...
4. **输入框**：v1 只发纯文本，不带附件，单条上限 8000 字。发送后立刻显示「已送达电脑」，随后进入进展流。
...
| 模型看到调用方会话 id 后自行回消息过去 | 实测三次都没有发生；列入 V-12 观察。万一发生，就是往一条无关会话里写了内容，所以 D-3 优先借最近活跃、内容无关紧要的会话 |
...
| D-3 | 调用方会话用哪条 | 用目标会话之外、最近活跃的那条会话 id；没有就提示用户先在桌面上随便开一条 | 默认 |
...
- **审批**：`wait_threads` 能得知「需要处理」，手机端先只提示「请到电脑上确认」。真正在手机上审批，只能走 CDP 或官方 remoteControl。
...
都是 v1 设计依赖、但还没亲手测过的行为。每项需要真跑一轮，会消耗少量额度。

| 编号 | 验证什么 | 影响哪里 |
|---|---|---|
| V-1 | 目标会话**正在跑**时再 `send_message_to_thread`：排队、并入当前一轮，还是报错？ | 手机端「发送」按钮在运行中是否可用 |
| V-2 | 目标会话是 `notLoaded`（桌面没打开过）时发送：会不会自动加载，界面会不会跳过去？ | 勾选区里 `notLoaded` 的会话能否直接勾选并使用 |
| V-3 | 调用方会话被归档或删除后，调用是否失败 | D-3 的兜底逻辑 |
| V-4 | 同一条会话，委派来的消息和用户亲手发的，在界面上是否确实完全无法区分 | D-4 |
| V-5 | 老会话的模型问题（§3.3-5）：`model` 覆盖为什么没生效？桌面上亲手发是否一样失败？ | 手机端的错误提示文案 |
| V-6 | 「自动」会话从手机发起、遇到审批时，rollout 里记什么、`list_threads` 的状态是什么；电脑上点确认后这一轮能否正常续跑 | 手机上「请到电脑上确认」的提示能否做；§3.5 的「卡住后回电脑确认」是否成立 |
| V-7 | 桌面版开多个窗口时，管道是一个还是多个 | 管道发现逻辑 |
| V-8 | 新建会话后，rollout 路径要等多久才出现在 state sqlite 里 | `SessionContentSync` 找文件时的重试 |
| V-9 | 18.6 MB 的 rollout 建一次轮次索引要多久、占多少内存；上下文压缩或 fork 之后 rollout 路径会不会变 | `open` 的首次延迟；`resync` 的触发条件 |
| V-10 | 一轮运行中，`read_thread` 能不能返回尚未完成的步骤（比如正在跑的命令） | 能否补上 §6.8 的「看不到正在执行的那一步」 |
| V-11 | `FileSystemWatcher` 对 `~/.codex/sessions` 下的追加写是否可靠触发 | 实时性，以及轮询间隔要设多短 |
| V-12 | 模型会不会顺着 `source_thread_id` 往调用方会话回消息（例如指令里写「完成后告诉我」） | D-3 选哪条会话当调用方 |

## 13. 任务拆分

| 编号 | 内容 | 依赖 |
|---|---|---|
| R0 | 跑完 §12 的 V-1 ~ V-12，把结论回填本文 | — |
