...
/// These types were only found because the trim analyzer was switched on for this
/// project: none of them appears in the server contracts, so the audit of the
/// transport layer missed all of them.
/// </para>
/// <para>
/// What they hold is worth noticing before dismissing them as unimportant — the
/// announcement read-state, the remembered Codex account, the selected group, the
/// last email that signed in, and the update manifest. Under a trimmed build with
/// reflection binding, these fail as silently as the wire contracts would:
/// announcements re-notify as if unread, the group preference resets to the
/// default on every launch, and the update check stops finding new versions.
/// Nothing crashes; the client just quietly forgets things.
/// </para>
/// <para>
/// The nested state records are <c>internal</c> rather than <c>private</c> for one
...
[JsonSerializable(typeof(LastAccountPreferenceStore.LastAccount), TypeInfoPropertyName = "LastAccount")]
