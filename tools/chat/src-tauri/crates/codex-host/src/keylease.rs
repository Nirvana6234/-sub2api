//! ⚠️ **已经是死代码（A9′，2026-09-03）。**
//!
//! 分组不再绑在 key 上：codex 只看得见 [`crate::LocalRelay`]，转发层拿账号会话
//! 打到 `/api/v1/paw/responses`，分组按请求选。客户端已经**不需要任何 API key**。
//!
//! 暂不删：新那条路还没对真中转站验过，留着回滚容易。**不要再接回去** ——
//! 接回去就是把分组又绑回 key 上，那正是这一版修掉的坑。
