export default {
    settings: {
      ...
      defaults: {
        ...
        accountShareRewardRate: '共享账号贡献奖励比例',
        accountShareRewardRateHint: '其他用户使用共享账号后，返给账号贡献者的贡献额度比例（0-100%，例如填写 80 表示返还实际消费的 80%）。修改后仅影响新的结算记录。',
        accountOwnUsageFeeRate: '自用贡献账号的平台抽成比例',
        accountOwnUsageFeeRateHint: '用户使用自己贡献的账号时，平台收取的比例（0-100%，默认 1%）。优先扣贡献额度，不足部分再扣站内余额。',
      ...
      openaiExperimentalScheduler: {
        ...
        latencyFallbackTitle: '延迟感知兜底',
        latencyFallbackDescription: '按最近首包延迟将账号分层，并仅在源分组偏慢且兜底分组明显更快时切换。样本不足时不触发。',
        latencyThresholdLabel: '偏慢阈值（毫秒）',
        speedupRatioLabel: '兜底最大延迟比例',
        ...
        stickyWeightedTitle: '普通会话粘性加权',
        stickyWeightedDescription: '开启后 session_hash 仅作为调度加分，相同绑定不滚动续期；previous_response_id 始终保持原账号。关闭后 session_hash 沿用硬粘性。',
