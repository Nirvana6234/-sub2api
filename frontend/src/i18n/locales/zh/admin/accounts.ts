export default {
    accounts: {
      ...
      upstreamBilling: {
        ...
        manual: '手动',
        manualRate: '管理员手动倍率：{value}x',
        manualRateHint: '自动探测继续记录，但不会覆盖此倍率。',
        setManualRate: '设置手动上游倍率',
        clearManualRate: '清除手动上游倍率',
        clearManualRateConfirm: '清除手动上游倍率并恢复使用探测结果？',
        manualRatePrompt: '输入手动上游倍率（不小于 0）',
        invalidManualRate: '请输入不小于 0 的有效倍率',
        manualRateSaved: '手动上游倍率已保存',
        manualRateCleared: '手动上游倍率已清除',
        manualRateSaveFailed: '保存手动上游倍率失败',
        stale: '已过期',
        unsupported: '不支持',
        failed: '失败',
        notProbed: '未探测',
      ...
      openai: {
        ...
        oauthPassthroughStrict: '字节保真模式（仅官方 Codex）',
        oauthPassthroughStrictDesc:
          '在自动透传之上再取消网关侧的形状兜底：请求头改为黑名单放行、出站体按官方客户端的方式压缩。这不是「更快的透传」，而是让上游看到的请求尽量贴近官方 Codex 客户端。必须同时开启上面的「仅允许 Codex 官方客户端」，否则非官方客户端会带着自己的头直达上游——这也是保存时会被拦下的原因。计费、并发、审计、身份影射与 turn-state 守卫一律不变。',
        oauthPassthroughStrictRequiresCLIOnly:
          '字节保真模式必须同时开启「仅允许 Codex 官方客户端」：前者取消的那些兜底，唯一的安全依据就是后者保证请求确实来自官方客户端。',
