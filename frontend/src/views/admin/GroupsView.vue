...
const fallbackBadge = (
  ...
  const names = targetIds.map((id) => groups.value.find((g) => g.id === id)?.name ?? `#${id}`);
...
const fallbackGroupCandidates = (sourcePlatform: string, claudeCodeOnly: boolean, currentId?: number) =>
  groups.value.filter((g) => {
    if (g.id === currentId || g.status !== "active") return false;
    if (usesFallbackPoolSelector(sourcePlatform) || (sourcePlatform === "anthropic" && !claudeCodeOnly)) {
...
const editFallbackGroupCandidates = computed(() =>
  fallbackGroupCandidates(editForm.platform, editForm.claude_code_only, editingGroup.value?.id),
);
...
const handleEdit = async (group: AdminGroup) => {
  ...
  editForm.is_fallback_pool = group.is_fallback_pool ?? false;
  ...
  editForm.fallback_group_ids = Array.from(new Set(group.fallback_group_ids?.length ? group.fallback_group_ids : (group.fallback_group_id ? [group.fallback_group_id] : [])));
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      // Legacy clients still receive the first target through the scalar field.
      fallback_group_id: editForm.fallback_group_ids[0] ?? 0,
