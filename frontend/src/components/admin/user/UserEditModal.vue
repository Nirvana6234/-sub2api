...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user', concurrency: 1, rpm_limit: 0, account_management_enabled: false, contribution_rooms_enabled: false, headroom_compression_enabled: false, customAttributes: {} as UserAttributeValuesMap })

watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', role: u.role || 'user', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, account_management_enabled: u.account_management_enabled, contribution_rooms_enabled: u.contribution_rooms_enabled, headroom_compression_enabled: u.headroom_compression_enabled, customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    const data: any = { email: form.email, username: form.username, notes: form.notes, role: form.role, concurrency: form.concurrency, rpm_limit: form.rpm_limit, account_management_enabled: form.account_management_enabled, contribution_rooms_enabled: form.contribution_rooms_enabled, headroom_compression_enabled: form.headroom_compression_enabled }
