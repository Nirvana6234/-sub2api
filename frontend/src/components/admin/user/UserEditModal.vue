  ...
  >
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <div class="space-y-3">
        ...
        <label class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-300">
          <input v-model="form.recharge_disabled" type="checkbox" class="mt-0.5 h-4 w-4" />
          <span>
            <span class="block font-medium">{{ t('admin.users.form.rechargeDisabled') }}</span>
            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ t('admin.users.form.rechargeDisabledHint') }}</span>
          </span>
        </label>
      </div>
...
const form = reactive({
  ...
  recharge_disabled: false,
  customAttributes: {} as UserAttributeValuesMap
})

watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', role: u.role || 'user', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, account_management_enabled: u.account_management_enabled, contribution_rooms_enabled: u.contribution_rooms_enabled, recharge_disabled: u.recharge_disabled === true, customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    const data: any = { email: form.email, username: form.username, notes: form.notes, role: form.role, concurrency: form.concurrency, rpm_limit: form.rpm_limit, account_management_enabled: form.account_management_enabled, contribution_rooms_enabled: form.contribution_rooms_enabled, recharge_disabled: form.recharge_disabled }
