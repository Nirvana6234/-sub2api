<template>
  ...
  >
    ...
    >
      ...
      <!-- Account-level custom outbound headers (all account types) -->
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="mb-3 flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.customHeaders.title') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <button
            type="button"
            @click="customHeadersEnabled = !customHeadersEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              customHeadersEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                customHeadersEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
        <div v-if="customHeadersEnabled" class="space-y-3">
          <div class="rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
            <p class="text-xs text-blue-700 dark:text-blue-400">
              <Icon name="exclamationCircle" size="sm" class="mr-1 inline" :stroke-width="2" />
              {{ t('admin.accounts.customHeaders.info') }}
            </p>
          </div>
          <HeaderOverrideEditor
            :rows="customHeadersRows"
            @update:rows="customHeadersRows = $event"
          />
        </div>
      </div>
...
const customHeadersRows = ref<HeaderOverrideRow[]>([])

const validateCustomHeaders = (): boolean => {
  if (!customHeadersEnabled.value) return true
  const headerError = validateHeaderOverrideRows(customHeadersRows.value)
  if (headerError) {
...
const applyCustomHeadersToExtra = (extra?: Record<string, unknown>): Record<string, unknown> | undefined => {
  const result: Record<string, unknown> = { ...(extra || {}) }
  if (customHeadersEnabled.value) {
    const customHeaders = buildHeaderOverridesObject(customHeadersRows.value)
    if (Object.keys(customHeaders).length > 0) {
      result.custom_headers = customHeaders
    } else {
      delete result.custom_headers
    }
  } else {
    delete result.custom_headers
  }
...
const handleSubmit = async () => {
  ...
  const extra = applyCustomHeadersToExtra(buildAnthropicExtra(buildOpenAIExtra()))
...
const createAccountAndFinish = async (
  ...
  finalExtra = applyCustomHeadersToExtra(finalExtra)
  if (platform === 'openai') {
    if (type === 'apikey') {
      applyOpenAIEndpointCapabilities(credentials)
    }
  ...
  try {
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        ...
        const extra = applyCustomHeadersToExtra(grokOAuth.buildExtraInfo(tokenInfo))
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  try {
    ...
    const extra = applyCustomHeadersToExtra(buildOpenAIExtra(oauthExtra))
...
const handleOpenAIImportCodexSession = async (content: string) => {
  ...
  try {
    const extra = applyCustomHeadersToExtra(buildOpenAICodexImportExtra())
    ...
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        ...
        const createPayload = withAntigravityConfirmFlag({
          ...
          type: 'oauth',
          credentials,
          extra: applyCustomHeadersToExtra({}),
          proxy_id: form.proxy_id,
          concurrency: form.concurrency,
          load_factor: form.load_factor ?? undefined,
          priority: form.priority,
