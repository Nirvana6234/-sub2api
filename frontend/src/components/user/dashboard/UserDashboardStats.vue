<template>
  <!-- Row 1: Core Stats -->
  <div class="grid grid-cols-2 gap-4 tabular-nums lg:grid-cols-4">
    <!-- Balance -->
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <span class="text-purple-600 dark:text-purple-400" :title="costTitle('actual', stats?.today_actual_cost || 0)">${{ formatCardCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-sm font-normal text-gray-400 dark:text-gray-500" :title="costTitle('standard', stats?.today_cost || 0)"> / ${{ formatCardCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-purple-600 dark:text-purple-400" :title="costTitle('actual', stats?.total_actual_cost || 0)">${{ formatCardCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-gray-400 dark:text-gray-500" :title="costTitle('standard', stats?.total_cost || 0)"> / ${{ formatCardCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
  ...
  <div v-if="!isSimple && showPlatformBreakdown" class="card p-4" data-testid="platform-breakdown">
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-purple-600 dark:text-purple-400" :title="costTitle('actual', item.total_actual_cost)">
            ${{ formatCardCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 dark:text-white" :title="costTitle('actual', item.today_actual_cost)">${{ formatCardCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
...
// 只有一张卡且没配额时，它展示的数字和第一行完全重复，只会多占一整行，收起。
const showPlatformBreakdown = computed(() =>
  platformCards.value.length > 1 || platformCards.value.some((card) => !card.isOther && hasAnyLimit(card.quota)),
)
...
// 卡片展示用：≥ 0.01 保留两位；更小的非零金额保留 4 位，免得显示成 $0.00 误以为没扣费。
const formatCardCost = (c: number) => (c === 0 || Math.abs(c) >= 0.01 ? c.toFixed(2) : c.toFixed(4))
const costTitle = (kind: 'actual' | 'standard', c: number) => `${t(`dashboard.${kind}`)}: $${formatCost(c)}`
