...
describe('playground persistence', () => {
  ...
  it('trims only an oversized conversation and leaves sibling history intact', () => {
    const oversized = createConversation({
      id: 'oversized',
      title: 'Large chat',
      messages: Array.from({ length: 48 }, (_, index) => createMessage('user', `message-${index}-${'x'.repeat(8000)}`)),
    })
  ...
  it('drops the oldest conversations when the complete browser snapshot exceeds 1 MiB', () => {
  ...
  it('flushes the captured key state before a rapid key switch', () => {
    vi.useFakeTimers()
    try {
