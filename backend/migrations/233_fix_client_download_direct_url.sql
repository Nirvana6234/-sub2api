...
--
-- 新地址是 downloads.example.com（your-server.example.com），由该机 nginx 以静态文件直接返回。
...
-- 「阻止不安全下载」拦。
--
-- 别和 app.example.com 混淆：那个域名解析到应用服务器 your-server.example.com，
...
UPDATE settings
SET value = 'https://downloads.example.com/downloads/codex-relay-client_v0.1_x64.zip',
    updated_at = NOW()
