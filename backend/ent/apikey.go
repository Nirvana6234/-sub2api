...
type APIKey struct {
	...
	// Automatically select the lowest-rate available group for this API key
	AutoGroup bool `json:"auto_group,omitempty"`
	// Auto-group strategy: price, balanced, or speed
	AutoGroupStrategy string `json:"auto_group_strategy,omitempty"`
...
type APIKeyEdges struct {
	...
	// ContributionRoomPreferences holds the value of the contribution_room_preferences edge.
	ContributionRoomPreferences []*UserContributionRoomPreference `json:"contribution_room_preferences,omitempty"`
...
// ContributionRoomPreferencesOrErr returns the ContributionRoomPreferences value or an error if the edge
// was not loaded in eager-loading.
func (e APIKeyEdges) ContributionRoomPreferencesOrErr() ([]*UserContributionRoomPreference, error) {
	if e.loadedTypes[2] {
		return e.ContributionRoomPreferences, nil
	}
	return nil, &NotLoadedError{edge: "contribution_room_preferences"}
}
	...
	for i := range columns {
		...
		case apikey.FieldAutoGroup:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field auto_group", values[i])
			} else if value.Valid {
				_m.AutoGroup = value.Bool
			}
		case apikey.FieldAutoGroupStrategy:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field auto_group_strategy", values[i])
			} else if value.Valid {
				_m.AutoGroupStrategy = value.String
			}
...
// QueryContributionRoomPreferences queries the "contribution_room_preferences" edge of the APIKey entity.
func (_m *APIKey) QueryContributionRoomPreferences() *UserContributionRoomPreferenceQuery {
	return NewAPIKeyClient(_m.config).QueryContributionRoomPreferences(_m)
}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("auto_group_strategy=")
	builder.WriteString(_m.AutoGroupStrategy)
	builder.WriteString(", ")
