...
func (APIKey) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("group_id").
			Optional().
			Nillable(),
		field.Bool("auto_group").
			Default(false).
			Comment("Automatically select the lowest-rate available group for this API key"),
		field.String("auto_group_strategy").
			MaxLen(20).
			Default("price").
			Comment("Auto-group strategy: price, balanced, or speed"),
		field.String("status").
			MaxLen(20).
...
func (APIKey) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("group", Group.Type).
			...
			Field("group_id").
			Unique(),
		edge.To("contribution_room_preferences", UserContributionRoomPreference.Type),
