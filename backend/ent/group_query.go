...
type GroupQuery struct {
	...
	withCompositeModelRoutes *CompositeModelRouteQuery
...
// QueryCompositeModelRoutes chains the current query on the "composite_model_routes" edge.
func (_q *GroupQuery) QueryCompositeModelRoutes() *CompositeModelRouteQuery {
	query := (&CompositeModelRouteClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withCompositeModelRoutes: _q.withCompositeModelRoutes.Clone(),
...
// WithCompositeModelRoutes tells the query-builder to eager-load the nodes that are connected to
// the "composite_model_routes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithCompositeModelRoutes(opts ...func(*CompositeModelRouteQuery)) *GroupQuery {
	query := (&CompositeModelRouteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCompositeModelRoutes = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	var (
		...
		loadedTypes = [9]bool{
			...
			_q.withCompositeModelRoutes != nil,
	...
	if query := _q.withCompositeModelRoutes; query != nil {
		if err := _q.loadCompositeModelRoutes(ctx, query, nodes,
			func(n *Group) { n.Edges.CompositeModelRoutes = []*CompositeModelRoute{} },
			func(n *Group, e *CompositeModelRoute) {
				n.Edges.CompositeModelRoutes = append(n.Edges.CompositeModelRoutes, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadCompositeModelRoutes(ctx context.Context, query *CompositeModelRouteQuery, nodes []*Group, init func(*Group), assign func(*Group, *CompositeModelRoute)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.CompositeModelRoute(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.CompositeModelRoutesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.group_composite_model_routes
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_composite_model_routes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_composite_model_routes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
