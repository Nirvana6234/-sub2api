...
func (s *OpenAIGatewayService) fetchOpenAIModelsUpstream(ctx context.Context, request openAIModelsRequest, ifNoneMatch string) (*OpenAIModelsResponse, error) {
	...
	restrictedAccount := requiresHTTPUpstreamPublicPolicy(request.account) ||
		requiresHTTPUpstreamPublicPolicy(request.credentialAccount)
	if request.useAPIKeyUpstream || restrictedAccount {
		...
		req = req.WithContext(WithHTTPUpstreamPublicHostsOnlyForAccount(req.Context(), request.account))
		if request.credentialAccount != request.account {
			req = req.WithContext(WithHTTPUpstreamPublicHostsOnlyForAccount(req.Context(), request.credentialAccount))
		}
