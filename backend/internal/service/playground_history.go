...
const (
	...
	// PlaygroundHistoryMaxBytes leaves a small margin above the browser's 96 KiB
	// compaction limit while still bounding one user's stored snapshot.
	PlaygroundHistoryMaxBytes         = 128 * 1024
...
func validatePlaygroundHistoryState(state json.RawMessage) (json.RawMessage, error) {
	...
	for _, rawConversation := range conversations {
		...
		for _, rawMessage := range messages {
			...
			if err := validateOptionalString(message, "content", 12000); err != nil {
				return nil, err
			}
			if err := validateOptionalString(message, "reasoningContent", 12000); err != nil {
				return nil, err
			}
		}
	}
