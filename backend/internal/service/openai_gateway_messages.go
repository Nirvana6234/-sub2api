...
type openAICompatBufferedReadError struct{ cause error }
...
// openAICompatTerminalResponse 从一个终止性流事件中取出可用的 ResponsesResponse。
//
// 失败类事件（response.failed / error）本身不带 Response 体，但下游仍然需要一个
// 结构化结果来完成用量结算和错误透传，所以这里就地合成一个 status=failed 的响应，
// 错误文案取自 SSE 负载，取不到时给一句兜底，避免把空消息透给客户端。
...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	// 入口分流：APIKey 账号 + 上游不支持 Responses API（含固定 chat_completions
	// 的 CN 账号）→ 走 CC 直转（与 ForwardAsChatCompletions 对称）。缺少此分流时，
	// /v1/messages 入站请求会被无条件转为 Responses 格式发往上游 /v1/responses，
	// 导致只支持 /v1/chat/completions 的第三方 OpenAI 兼容上游全部 400。
	if shouldForwardOpenAIResponsesViaRawChatCompletions(account) {
	...
	for attempt := 0; ; attempt++ {
		if attempt > 0 {
			...
			upstreamCtxRetry, releaseRetry := detachUpstreamRequestContext(ctx)
