...
func TestInjectSEOByRoute(t *testing.T) {
	baseHTML := []byte(`<html><head><title>Default</title><meta name="description" content="default"></head><body></body></html>`)
	settings := []byte(`{"site_name":"Test Site","client_download_enabled":true,"model_plaza_enabled":true}`)

	homeRequest := httptest.NewRequest(http.MethodGet, "https://example.com/home?from=test", nil)
	home := string(injectSEO(baseHTML, settings, homeRequest))
	assert.Contains(t, home, "<title>Sub2API - AI API 聚合网关与模型 API 服务</title>")
	assert.Contains(t, home, `content="index,follow"`)
	assert.Contains(t, home, `href="https://example.com/home"`)
	assert.Contains(t, home, `id="sub2api-seo-jsonld"`)

	plazaRequest := httptest.NewRequest(http.MethodGet, "https://example.com/model-plaza", nil)
	plaza := string(injectSEO(baseHTML, settings, plazaRequest))
	assert.Contains(t, plaza, "<title>AI 模型广场 - Claude、GPT、Gemini API</title>")
	assert.Contains(t, plaza, `https://example.com/model-plaza`)
	assert.NotEqual(t, home, plaza)

	privateRequest := httptest.NewRequest(http.MethodGet, "https://example.com/dashboard", nil)
	privatePage := string(injectSEO(baseHTML, settings, privateRequest))
	assert.Contains(t, privatePage, `content="noindex,nofollow"`)
	assert.NotContains(t, privatePage, `id="sub2api-seo-jsonld"`)
}

func TestFrontendServer_SEODoesNotLeakAcrossCachedRoutes(t *testing.T) {
	provider := &mockSettingsProvider{
		settings: map[string]any{
			"site_name":               "Test Site",
	...
	paths := []struct {
		...
		{"/home", "Sub2API - AI API 聚合网关与模型 API 服务"},
		{"/model-plaza", "AI 模型广场 - Claude、GPT、Gemini API"},
		{"/dashboard", "content=\"noindex,nofollow\""},
	}
	for _, test := range paths {
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "https://example.com"+test.path, nil)
...
func TestFrontendServer_SEOFiles(t *testing.T) {
	...
	assert.Contains(t, robotsWriter.Body.String(), "Sitemap: https://example.com/sitemap.xml")
	...
	assert.Contains(t, sitemapWriter.Body.String(), "<loc>https://example.com/home</loc>")
	assert.Contains(t, sitemapWriter.Body.String(), "<loc>https://example.com/model-plaza</loc>")
	assert.Contains(t, sitemapWriter.Body.String(), "<loc>https://example.com/download</loc>")
}

func TestFrontendServer_SitemapOnlyListsIndexablePages(t *testing.T) {
	tests := []struct {
		name     string
		settings map[string]any
		includes []string
		...
		{
			name: "excludes_everything_in_backend_mode",
			settings: map[string]any{
				"backend_mode_enabled":    true,
				"client_download_enabled": true,
				"model_plaza_enabled":     true,
			},
			excludes: []string{"/home", "/model-plaza", "/download"},
		},
		{
			name: "excludes_disabled_or_authenticated_public_pages",
			settings: map[string]any{
				"client_download_enabled":  false,
				"model_plaza_enabled":      true,
				"model_plaza_require_auth": true,
				"home_content":             "https://example.com/embedded-home",
			},
			excludes: []string{"/home", "/model-plaza", "/download"},
		},
		{
			name: "keeps_only_enabled_public_pages",
			settings: map[string]any{
				"client_download_enabled": true,
				"model_plaza_enabled":     false,
			},
			includes: []string{"/home", "/download"},
			excludes: []string{"/model-plaza"},
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			server, err := NewFrontendServer(&mockSettingsProvider{settings: test.settings})
			require.NoError(t, err)

			router := gin.New()
			router.Use(server.Middleware())
			writer := httptest.NewRecorder()
			request := httptest.NewRequest(http.MethodGet, "https://example.com/sitemap.xml", nil)
			router.ServeHTTP(writer, request)

			assert.Equal(t, http.StatusOK, writer.Code)
			for _, path := range test.includes {
				assert.Contains(t, writer.Body.String(), "<loc>https://example.com"+path+"</loc>")
			}
			for _, path := range test.excludes {
				assert.NotContains(t, writer.Body.String(), "<loc>https://example.com"+path+"</loc>")
			}
		})
	}
}
